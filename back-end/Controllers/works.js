...
exports.send_work = async(req, res) => {
    try {
        ...
        const sendDateInBangkok = new Date(sendDate.toLocaleString('en-US', { timeZone: 'Asia/Bangkok' }));
        ...
        const queryCheckType = `SELECT a.assignment_type,w.id_group,a.id_classroom,a.colses_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok' FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
            WHERE w.id = $1
        ...
        console.log("date1 ",sendDateInBangkok.getTime()," date2 ",date_closes.getTime())
        if(date_closes.getTime() !== 0){
        if(sendDateInBangkok.getTime() > date_closes.getTime()){
            if (req.files) {
