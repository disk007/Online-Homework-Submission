...
exports.send_work = async(req, res) => {
    try {
        ...
        if(io){
            io.to(String(queryCheckType.rows[0].id_classroom)).emit("activity-send")
        }
...
exports.cancel_work = async (req, res) => {
    try {
        ...
        let querySql
        if(resultCheckType.rows[0].assignment_type === 'group'){
            querySql = `UPDATE work SET is_submitted = $1 ,sent_date = $2,verify = $3,activity = 'Initial' FROM groups_member AS gm
                WHERE 
                    gm.id_group = work.id_group
                    AND gm.id_user = $4
                    AND work.id = $5`
            ...
            querySql = `UPDATE work SET is_submitted = $1,sent_date = $2,verify = $3,activity = 'Initial' WHERE id_user = $4 AND id = $5 `
        }
