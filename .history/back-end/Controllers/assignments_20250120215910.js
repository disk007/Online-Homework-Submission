    ...
    try{
        ...
        if(typeWork === 'All students'){
            ...
            if(check_groups_members.rows.length === 0){
                ...
                const findIdGroupRemember = await db.query("SELECT * FROM groups_remember WHERE id_classroom = $1",[idRoomFilter[0]])
                await Promise.all(
                    membersArray.map(async (group, i) => {
                        // ใช้ Promise.all ภายใน group.members.map เพื่อจัดการ async
                        await Promise.all(
                            group.members.map(async (member) => {
                                ...
                                await db.query(
                                    "UPDATE groups_remember SET id_user = $1,id_group = $2 WHERE id_classroom = $3",
                                    [member.id_user, idGroups[i], idRoomFilter[0]]
                                );
                            })
                        );
