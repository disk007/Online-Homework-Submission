...
exports.update_assignment = async(req,res) => {
    try {
        const {title,instructions,score,due_time,colses_time,assignmentId} = req.body
        console.log(title)
        ...
        const dueDateTime = `${due_time.getFullYear()}-${(due_time.getMonth() + 1).toString().padStart(2, '0')}-${due_time.getDate().toString().padStart(2, '0')} ${due_time.getHours().toString().padStart(2, '0')}:${due_time.getMinutes().toString().padStart(2, '0')}:00`
