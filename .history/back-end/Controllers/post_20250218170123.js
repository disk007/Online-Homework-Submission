...
exports.get_post = async (req, res) => {
    try {
        ...
        const querySql = `SELECT p.id_user,p.id, p.message, p.file, p.create_at,p.update_create, CONCAT(u.fname, ' ', u.lname) AS name ,u.role FROM post AS p
...
exports.add_comment = async (req, res) => {
    try {
        ...
        const id_classroom = await db.query("SELECT id_classroom FROM post WHERE id = $1",[id_post])
        const io = req.app.get("io");
        if(io){
...
exports.get_comment = async (req, res) => {
    try {
        const {id_classroom} = req.params
        const querySql = `SELECT c.id_user,c.id_post,c.id,c.message,c.create_at,c.update_create,CONCAT(u.fname, ' ', u.lname)AS name FROM comment AS c
...
exports.edit_post = async (req, res) => {
    try {
        ...
        const convertDate = new Date(date)
        const curentDate = `${convertDate.getFullYear()}-${(convertDate.getMonth() + 1).toString().padStart(2, '0')}-${convertDate.getDate().toString().padStart(2, '0')} ${convertDate.getHours().toString().padStart(2, '0')}:${convertDate.getMinutes().toString().padStart(2, '0')}:00`
        const sqlFileName = await db.query('SELECT file,id_classroom FROM post WHERE id = $1 ',[id])
        let result
        ...
        if(fileName != 'null'){
            ...
            const querySql = `UPDATE post SET message = $1 , file = $2 , update_create = $3 WHERE id = $4`
            result = await db.query(querySql,[message,filterFileName,curentDate,id])
        }
        else{
            const querySql = `UPDATE post SET message = $1 , update_create = $2 WHERE id = $3`
...
exports.edit_comment = async (req, res) => {
    try {
        const {id,message,date,classroomId} = req.body
         const convertDate = new Date(date)
        const curentDate = `${convertDate.getFullYear()}-${(convertDate.getMonth() + 1).toString().padStart(2, '0')}-${convertDate.getDate().toString().padStart(2, '0')} ${convertDate.getHours().toString().padStart(2, '0')}:${convertDate.getMinutes().toString().padStart(2, '0')}:00`
        await db.query("UPDATE comment SET message = $1,update_create = $2  WHERE id = $3",[message,curentDate,id])
        const io = req.app.get("io");
        if(io){
            io.to(String(classroomId)).emit("get-comment")
        }
        return res.json({status:'success'})
    } catch (error) {
        console.log(error)
