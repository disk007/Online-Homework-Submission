...
exports.my_work = async(req,res) => {
    try {
        const {user_id} = req.params
        const querySql = `SELECT a.id AS id_assignment,a.due_time,a.colses_time,w.work,w.verify,w.feedback,w.id,w.is_submitted,w.sent_date,w.id_group,(a.score) AS Ascore,(w.score) AS Wscore FROM assignment AS a
...
exports.cancel_work = async (req, res) => {
    try {
        ...
        if(resultCheckType.rows[0].assignment_type === 'group'){
            ...
            querySql = `UPDATE work SET is_submitted = $1,sent_date = $2,verify = $3 WHERE id_user = $3 AND id = $4 `
        }
