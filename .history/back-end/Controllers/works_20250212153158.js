...
exports.detail_work = async(req,res)=>{
    try {
        const {work_id,id_user} = req.params
        ...
        ORDER BY 
            a.id DESC`
        const result = await db.query(querySql,[work_id,id_user])
...
exports.my_work = async(req,res) => {
    try {
        const {id_work,user_id} = req.params
        ...
        const result = await db.query(querySql,[id_work,user_id])
        return res.json(result.rows)
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: error.message });
...
exports.update_activity = async(req, res) => {
    try {
        const {id} = req.body
        const querySql = `UPDATE work SET activity = 'Viewed' WHERE id = $1`
