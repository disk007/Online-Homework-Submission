...
exports.activity_teacher = async (req,res) => {
    try{
        ...
        const querySql = `SELECT w.id_assignment,a.id_classroom,COUNT(w.id_assignment),w.id,w.id_group,c.name,a.title FROM work AS w
        INNER JOIN assignment AS a ON a.id = w.id_assignment
        INNER JOIN classroom AS c ON c.id = a.id_classroom
        WHERE c.id_user = 3 AND w.is_submitted = true
        GROUP BY w.id_assignment,w.id,a.id_classroom,w.id_group,c.name,a.title`
        const result = await db.query(querySql,[id])
        return res.json(result.rows)
    }
    catch (error) {
