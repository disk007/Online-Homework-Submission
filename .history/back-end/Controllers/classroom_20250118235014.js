...
exports.join_classroom = async (req, res) => {
    try{
        ...
        const queryAss = `SELECT a.id FROM assignment AS a
            INNER JOIN classroom AS c ON c.id = a.id_classroom
            WHERE c.id = $1 AND a.assignment_type = 'All students'`
        const resultAss = await db.query(queryAss,[check_code.rows[0].id])
        let idWork = null;
        if(resultAss.rows.length > 0){
            const queryidWork = `SELECT a.id AS assignment_id, u.id AS user_id
                FROM assignment AS a
                INNER JOIN classroom AS c ON c.id = a.id_classroom
                INNER JOIN members AS m ON m.id_classroom = c.id
                INNER JOIN users AS u ON u.id = m.id_user
                LEFT JOIN work AS w ON w.id_assignment = a.id AND w.id_user = u.id
                WHERE a.id = $1 AND w.id IS NULL `
                idWork = await Promise.all(
                    resultAss.rows.map(async (d, i) => {
                        const result = await db.query(queryidWork,[d.id])
                        return result.rows;
                    })
                )
        }
        const flattenedIdWork = idWork.flat();
        flattenedIdWork.map((work) => {
