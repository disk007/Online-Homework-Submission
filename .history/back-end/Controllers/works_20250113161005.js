    ...
    try {
        ...
        if(assignment_type === 'group'){
            idArray.map(async (d,i) => {
                await db.query('UPDATE work SET feedback = $1 , score = $2 , verified = $3 WHERE id_group = $4 AND id = $5',[feedbackArray[i],scoreArray[i],true,d,id_work])
            })
        }
        else{
            idArray.map(async (d,i) => {
                await db.query('UPDATE work SET feedback = $1 , score = $2 , verified = $3 WHERE id_user = $4 AND id = $5',[feedbackArray[i],scoreArray[i],true,d,id_work])
            })
        }
