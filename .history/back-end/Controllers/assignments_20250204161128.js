    ...
    try{
        ...
        if(typeWork === 'All students'){
            ...
            if(check_groups_members.rows.length === 0){
                ...
                await db.query(
                    "INSERT INTO groups_remember (id_user, id_group, id_classroom,name) VALUES " +
                        insertData.map(() => "($1, $2, $3, $4)").join(", "),
                    insertData.flat()
                );
                ...
                // ใช้ bulk insert เพื่อลดจำนวน query
                await db.query(
                    "INSERT INTO groups_remember (id_user, no_group, id_classroom,name) VALUES " +
                        insertData.map(() => "($1, $2, $3, $4)").join(", "),
                    insertData.flat()
                );
            }
            ...
            idAssignmentWork = await Promise.all(
                insertIdWorks.map(async(data)=>{
                    ...
                    const result = await db.query(`SELECT id, no_group, id_assignment FROM work WHERE id = $1 `,[data])
                    return result.rows;
                })
            ) 
        }
