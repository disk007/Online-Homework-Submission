...
exports.delete_classroom  = async(req,res) => {
    try {
        ...
        const querySql = `SELECT g.id AS gId,w.id AS wId,a.id AS aId FROM assignment AS a
        INNER JOIN work AS w ON w.id_assignment = a.id
        LEFT JOIN groups AS g ON g.id = w.id_group
        WHERE a.id_classroom = $1`
        const idGAW = await db.query(querySql,[classroomId])
        const uniqueGids = [...new Map(data
            .filter(item => item.gid !== null) // กรอง gid ที่ไม่ใช่ null
            .map(item => [item.gid, item]) // แปลงเป็น key-value
          ).values()];
        console.log(idGAW.rows)
        return res.json({status:'success'})
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: error.message });
