        ...
        if(typeWork === 'All students'){
            ...
            if(check_groups_members.rows.length === 0){
                membersArray.map(async (group) => {
                    ...
                    // const id = await db.query("INSERT INTO groups (name) VALUES ($1) RETURNING id",[group.name])
                    group.members.map(async (member) => {
                        await db.query("INSERT INTO groups_remember (id_user,id_group,id_classroom) VALUES ($1,$2,$3)",[member.id_user,idGroups[0],idRoomFilter[0]])
                    })
                ...
                membersArray.map(async (group) => {
                    ...
                    // const id = await db.query("INSERT INTO groups (name) VALUES ($1) RETURNING id",[group.name])
                    group.members.map(async (member) => {
                        await db.query("UPDATE groups_remember SET id_user = $1,id_group = $2 WHERE id_classroom = $3",[member.id_user,idGroups[0],idRoomFilter[0]])
                    })
