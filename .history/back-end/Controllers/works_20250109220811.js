    ...
    try {
        ...
        if(resultCheckType.rows[0].assignment_type === 'group'){
            if(fileName != 'null'){
                ...
                const querySql = `UPDATE work SET is_submitted = $1, work = $2 ,sent_date =$3 INNER JOIN groups_member AS gm ON gm.id_group = work.id_group WHERE gm.id_user = $4 AND work.id = $5 RETURNING id_assignment`
                result = await db.query(querySql,[true,filterFileName,formatDate,id_user,id_work])
            }
            else{
                const querySql = `UPDATE work SET is_submitted = $1 , sent_date = $2 INNER JOIN groups_member AS gm ON gm.id_group = work.id_group WHERE gm.id_user = $3 AND work.id = $4 RETURNING id_assignment`
        ...
        if (req.files && req.body) {
            req.files.forEach((file, index) => {
                ...
                const folderFile = path.join(__dirname,'../assignments',result.rows[0].id_assignment.toString(),resultCheckType.rows[0].assignment_type === 'group' ? id_work.toString() : id_work.toString(),id_user.toString());
