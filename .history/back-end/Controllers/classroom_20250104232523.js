...
exports.re_code = async(req,res) => {
    try{
        const {id_classroom} = req.body()
        let isUnique = false;
        while (!isUnique) {
            uniqueCode = generateCode();
            // Check if uniqueCode exists in the database
            const codeExists = await db.query('SELECT * FROM classroom WHERE code = $1 AND id = $2', [uniqueCode,id_classroom]);
            if (codeExists.rows.length === 0) {
                isUnique = true;  // If no rows are found, it means the code is unique
            }
        }
        await db.query('UPDATE classroom SET code = $1')
    }
    catch (error) {
        console.error('Error checking classroom access:', error);
        res.status(500).send('Server error');
...
exports.check_work_access = async(req,res) => {
    try {
        ...
        console.log("userId "+userId+" workId "+ workId)
        const isStudent = await db.query(
            'SELECT work.id FROM work INNER JOIN assignment ON assignment.id = work.id_assignment WHERE id_user = $1 AND work.id = $2 AND id_classroom = $3',
            [userId, workId,classroomId]
        );
        if (isStudent.rows.length > 0) {
            return res.json({ isAuthorized: true }); 
        } else {
...
exports.check_full_work_access = async(req,res) => {
    try {
        ...
        const isStudent = await db.query(
            'SELECT work.id FROM work WHERE id_user = $1 AND work.id = $2 ',
            [userId, workId]
        );
        if (isStudent.rows.length > 0) {
            return res.json({ isAuthorized: true }); 
