...
exports.forgot_password = async(req, res) => {
    try {
        ...
        const token = jwt.sign({email}, secretPassword, { expiresIn: "1h" });
        await db.query('UPDATE users SET reset_pass_token = $1 WHERE email = $2', [token, email]);
        ...
        const receiver = {
            ...
            <div style="max-width: 500px; margin: auto; padding: 20px; 
                ...
                <p style="margin-top: 15px; color: #d9534f; font-size: 14px; font-weight: bold;">
...
exports.reset_password = async(req, res) => {
    try{
        ...
        const result = await db.query('UPDATE users SET password = $1 , reset_pass_token = $2 WHERE email = $3', [hash, 'Expired', decoded.email]);
        if(!result){
            return res.status(500).json({status:'error', message: 'Failed to update password' });
        }
        return res.json({ status:'success'})
...
exports.verifyTokenPass = async (req, res) => {
    try {
        ...
        const decoded = jwt.verify(token, secretPassword);
        const check_token = await db.query('SELECT reset_pass_token FROM users WHERE email = $1',[decoded.email])
        if(check_token.rows[0].reset_pass_token === 'Expired'){
            return res.json({ valid: false});
        }
        // console.log(decoded);
