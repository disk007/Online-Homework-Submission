...
exports.join_classroom = async (req, res) => {
    try{
        ...
        if(resultAss.rows.length > 0){
            ...
            let dataWork = null
            const queryDataWork = `SELECT * FROM work AS w
            WHERE w = $1 `;
            dataWork = await Promise.all(
                flattenedIdWork.map(async (d, i) => {
                    const result = await db.query(queryDataWork,[d.id])
                    return result.rows;
                })
            )
            const flattenedDataWork = dataWork.flat();
            flattenedDataWork.map(async (d, i) => {
                await db.query('INSERT INTO work (work,feedback,score,)')
            })
        }
