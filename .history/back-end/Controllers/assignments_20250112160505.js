...
exports.detail_assignment = async (req, res) => {
    try {
        ...
        if(resultCheckIdwork.rows[0].assignment_type === 'group'){
            querySql = `SELECT w.id,w.id_group,g.name AS group_name,a.title,a.due_time,a.colses_time,w.is_submitted,w.work,STRING_AGG(u.fname || ' ' || u.lname, ', ') AS group_members FROM assignment AS a
            ...
            querySql = `SELECT w.id,w.id_user,a.title,a.due_time,a.colses_time,u.fname,u.lname,w.is_submitted,w.work FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
