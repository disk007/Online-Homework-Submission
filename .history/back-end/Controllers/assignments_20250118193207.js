...
exports.update_assignment = async(req,res) => {
    try {
        ...
        let closeDateTime = null;
        if(colses_time !== 'null'){
            let closeDateTime = new Date(colses_time);
            console.log("closeDateTime b "+closeDateTime)
            closeDateTime = `${closeDateTime.getFullYear()}-${(closeDateTime.getMonth() + 1).toString().padStart(2, '0')}-${closeDateTime.getDate().toString().padStart(2, '0')} ${closeDateTime.getHours().toString().padStart(2, '0')}:${closeDateTime.getMinutes().toString().padStart(2, '0')}:00`;
        }
        ...
        await db.query(updateAssignment,[title,instructions,score,dueDateTime,closeDateTime,filterFileName,assignmentId])
