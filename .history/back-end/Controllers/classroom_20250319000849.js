    ...
    try{
        ...
        if(resultAss.rows.length > 0){
            ...
            flattenedIdWork.map(async (d, i) => {
                console.log("d.assignment_id,d.id "+d.id_assignment,d.id)
...
exports.update_classroom = async (req, res) => {
    try {
        ...
        console.log(error);
        if (error.code === '23505') { // รหัส 23505 = duplicate key violation
            return res.json({ status: 'duplicate', message: 'Classroom name already exists' });
        }
        return res.status(500).json({ error: error.message });
    }
}
...
exports.delete_classroom  = async(req,res) => {
    try {
        ...
        uniqueAids.map(async(d,i)=>{
            await db.query('DELETE FROM work WHERE id_assignment = $1',[d])
        })
        uniqueAids.map(async(d,i)=>{
            await db.query('DELETE FROM assignment WHERE id = $1',[d])
        })
        uniquePids.map(async(d,i)=>{
            await db.query('DELETE FROM comment WHERE id_post = $1',[d])
        })
        uniqueGids.map(async(d,i)=>{
            await db.query('DELETE FROM work WHERE id_group = $1', [d]);
