...
exports.send_work = async(req, res) => {
    try {
        ...
        const queryCheckType = `SELECT a.assignment_type FROM assignment AS a
            ...
            GROUP BY a.id`
        let sqlFileName
        console.log(queryCheckType.rows[0].assignment_type)
        // if(queryCheckType.rows[0].assignment_type === 'group'){
        ...
        //         const folderFile = path.join(__dirname,'../assignments',result.rows[0].id_assignment.toString(),queryCheckType.rows[0].assignment_type === 'group' ? id_work.toString() : id_work.toString(),id_user.toString());
