...
exports.get_comment = async (req, res) => {
    try {
        const querySql = `SELECT c.id,c.message,c.create_at,CONCAT(u.fname, ' ', u.lname) FROM comment AS c
            INNER JOIN post AS p ON p.id = c.id_post
            INNER JOIN users AS u ON u.id = p.id_user
            WHERE p.id_classroom = $1`
        console.log("req.params.id_classroom "+req.params.id_classroom)
        const result = await db.query(querySql,[req.params.id_classroom])
        return res.json(result.rows)
    } catch (error) {
        console.log(error)
        return res.status(500).json({ error: error.message })
