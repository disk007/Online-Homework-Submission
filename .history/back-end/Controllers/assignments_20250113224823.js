...
exports.detail_assignment = async (req, res) => {
    try {
        ...
        if(resultCheckIdwork.rows[0].assignment_type === 'group'){
            querySql = `SELECT w.id,a.score,w.id_group,g.name AS group_name,a.title,a.due_time,a.colses_time,w.is_submitted,w.work,STRING_AGG(u.fname || ' ' || u.lname, ', ') AS group_members FROM assignment AS a
            ...
            querySql = `SELECT w.id,a.score,w.id_user,a.title,a.due_time,a.colses_time,u.fname,u.lname,w.is_submitted,w.work FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
...
exports.verified_assignment = async (req, res) => {
    try {
        ...
        const resultCheckIdwork = await db.query(queryCheckIdwork, [assignmentId])
        if(resultCheckIdwork.rows[0].assignment_type === 'group'){
            querySql = `SELECT w.feedback,w.score AS wscore,a.score AS ascore,w.id,w.id_group,g.name AS group_name,a.title,a.due_time,a.colses_time,w.is_submitted,w.work,STRING_AGG(u.fname || ' ' || u.lname, ', ') AS group_members FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
			INNER JOIN groups AS g ON g.id = w.id_group
			LEFT JOIN groups_member AS gm ON gm.id_group = g.id
			INNER JOIN users AS u ON u.id = gm.id_user
            WHERE a.id = $1 AND w.verify = true
			GROUP BY w.id,a.id,w.is_submitted,w.id_group,g.id,w.work,w.score`
        }
        else{
            querySql = `SELECT w.id,w.score AS wscore,a.score AS ascore,w.id_user,a.title,a.due_time,a.colses_time,u.fname,u.lname,w.is_submitted,w.work FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
            INNER JOIN users AS u ON u.id = w.id_user
            WHERE a.id = $1 AND w.verify = true`
        }
