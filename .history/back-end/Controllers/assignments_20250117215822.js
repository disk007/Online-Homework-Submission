...
exports.data_assignment = async (req, res) => {
    try {
        ...
        const querySql = `SELECT w.id,a.title,a.instructions,a.reference_files,a.score,a.due_time AS duetime,a.colses_time AS colsestime,a.assignment_type,c.name FROM assignment AS a
            INNER JOIN classroom AS c ON c.id = a.id_classroom
            INNER JOIN work AS w ON w.id_assignment = a.id 
            WHERE a.id = $1`;
