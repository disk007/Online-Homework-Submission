...
exports.update_assignment = async(req,res) => {
    try {
        ...
        if(colses_time !== 'null'){
            ...
            const close_DateTime = `${closeDateTime.getFullYear()}-${(closeDateTime.getMonth() + 1).toString().padStart(2, '0')}-${closeDateTime.getDate().toString().padStart(2, '0')} ${closeDateTime.getHours().toString().padStart(2, '0')}:${closeDateTime.getMinutes().toString().padStart(2, '0')}:00`;
            closeDateTime = close_DateTime
        }
        ...
        await db.query(updateAssignment,[title,instructions === "" ? null :instructions,score,dueDateTime,closeDateTime,filterFileName,assignmentId])
        ...
        if (req.files && req.body) {
            req.files.forEach((file, index) => {
                console.log("file.path "+file.path)
                console.log(file, fileName[index])
                const folderFile = path.join(__dirname,'../assignments',obj.assignmentId.toString(),obj.id.toString(),'file');
                if (!fs.existsSync(folderFile)) {
                    try {
                        fs.mkdirSync(folderFile, { recursive: true })
                        console.log(`Folder created: ${folderFile}`);
