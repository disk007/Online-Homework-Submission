            ...
            if(check_groups_members.rows.length === 0){
                ...
                membersArray.forEach((group, i) => {
                    ...
                    group.members.forEach((member) => {
                        insertData.push([member.id_user, groupIndex, idRoomFilter[0]]);
                    });
                });
            
                // ใช้ bulk insert เพื่อลดจำนวน query
                await db.query(
                    "INSERT INTO groups_remember (id_user, id_group, id_classroom) VALUES " +
                        insertData.map(() => "($1, $2, $3)").join(", "),
                    insertData.flat()
                );
                ...
                await Promise.all(
                    membersArray.map(async (group, i) => {
                        // ใช้ Promise.all ภายใน group.members.map เพื่อจัดการ async
                        await Promise.all(
                            group.members.map(async (member) => {
                                // console.log("members "+member.id_user,"  + " ,idGroups[i]," + "+idRoomFilter[0])
                                await db.query(
                                    "INSERT INTO groups_remember (id_user, id_group, id_classroom) VALUES ($1, $2, $3)",
                                    [member.id_user, idGroups[i], idRoomFilter[0]]
                                );
                            })
                        );
                    })
...
exports.activity_teacher = async (req,res) => {
    try{
        const {id} = req.params
        const querySql = `SELECT a.title,a.id AS id_assignment,a.id_classroom,w.id,c.name,u.fname,u.lname,w.sent_date FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
			INNER JOIN classroom AS c ON c.id = a.id_classroom
			INNER JOIN users AS u ON u.id = w.id_user
...
exports.remember_groups = async(req,res) => {
    try {
        ...
        const querySql = `SELECT gr.id_user,gr.no_group,g.name FROM groups_remember AS gr
        INNER JOIN groups AS g ON g.id = gr.id_group
    ...
    try {
        ...
        if(assignment_type === 'group'){
            ...
            if(findIdGroup.rows.length > 0){
                ...
                const deleteGroupRemember = 'DELETE FROM groups_remember WHERE id_group = $1'
                await Promise.all(
                    findIdGroup.rows.map(async (group) => {
                        await db.query(deleteGroupRemember,[group.id_group])
                        // console.log(group.id_group)
                    })
                )
                // await db.query(deleteGroup,[findIdGroup.rows[0].id_group])
            }
        }
        else{
            await db.query('DELETE FROM work WHERE work.id_assignment = $1',[assignmentId])
        ...
        const folderPath = path.join(__dirname,'../assignments',assignmentId.toString());
