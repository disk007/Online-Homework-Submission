...
exports.count_teacher_activity = async(req,res) => {
    try{
        const {user_id} = req.params
        const querySql = `w.activity = 'InitialT'`
    }
}
...
update_teacher_activity = async(req,res) => {
    try {
        const {id_assignment,id_classroom} = req.body
        const querySql = `UPDATE work SET activity = 'ViewedT' WHERE id_assignment = $1 AND id_classroom = $2 AND is_submitted = true`
...
exports.send_work = async(req, res) => {
    try {
        ...
        const queryCheckType = `SELECT a.assignment_type,w.id_group,a.id_classroom FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
            WHERE w.id = $1
