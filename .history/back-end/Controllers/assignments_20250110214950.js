...
exports.detail_assignment = async (req, res) => {
    try {
        ...
        if(resultCheckIdwork.rows[0].assignment_type === 'group'){
            querySql = `SELECT g.name AS groupName,a.title,a.due_time,a.colses_time,w.is_submitted,STRING_AGG(u.fname || ' ' || u.lname, ', ') AS group_members FROM assignment AS a
                ...
                WHERE a.id = $1
                GROUP BY w.id,a.id,w.is_submitted,w.id_group,g.id`
        }
        else{
            querySql = `SELECT a.title,a.due_time,a.colses_time,u.fname,u.lname,w.is_submitted FROM assignment AS a
            INNER JOIN work AS w ON w.id_assignment = a.id
        ...
        return res.json({"assignment_type":resultCheckIdwork.rows[0].assignment_type,result.rows});
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: error.message });
    }
