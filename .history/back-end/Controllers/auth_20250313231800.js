...
exports.forgot_password = async(req, res) => {
    try {
        ...
        await db.query('UPDATE users SET reset_pass_ = $1 WHERE email = $2', [hash, decoded.email]);
...
exports.reset_password = async(req, res) => {
    try{
        ...
        const result = await db.query('UPDATE users SET password = $1 WHERE email = $2', [hash, decoded.email]);
        if(!result){
            return res.status(500).json({status:'error', message: 'Failed to update password' });
        }
        return res.json({ status:'success'})
