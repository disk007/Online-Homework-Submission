...
exports.join_classroom = async (req, res) => {
    try{
        ...
        if(resultAss.rows.length > 0){
            ...
            //     WHERE w.id IS NULL
            //     AND a.id_classroom = $2
            // `;
            const queryidWork = `SELECT a.id AS assignment_id
                FROM assignment AS a
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM work AS w
                    WHERE w.id_assignment = a.id AND w.id_user = $1
                )
                AND a.id_classroom = $2`
                idWork = await Promise.all(
                    resultAss.rows.map(async (d, i) => {
                        const result = await db.query(queryidWork,[d.id,id_student])
                        return result.rows;
                    })
                )
        }
