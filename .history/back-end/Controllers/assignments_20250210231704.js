...
exports.add_assignments = async (req,res) => {
    try{
        ...
        if (io && Array.isArray(idRoomArray)) {
            idRoomArray.forEach((classroomId) => {
                io.to(String(classroomId)).emit("activityStudent", activityResult.rows);
            });
        } else if (io && idRoomArray) {
            io.to(String(idRoomArray)).emit("activityStudent", activityResult.rows);
        }
