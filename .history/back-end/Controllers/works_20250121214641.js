    ...
    try {
        ...
        if(resultCheckType.rows[0].assignment_type === 'group'){
            if(fileName != 'null'){
                ...
                const querySql = `UPDATE work SET is_submitted = $1, work = $2 ,sent_date =$3,verify = $4 FROM groups_member AS gm
                        WHERE 
                ...
                const querySql = `UPDATE work SET is_submitted = $1 , sent_date = $2,verify = $3 FROM groups_member AS gm
                        WHERE 
                            gm.id_group = work.id_group
                            AND gm.id_user = $4
            ...
            if(fileName != 'null'){
                existingFileNames = [...existingFileNames, ...fileName];
                const filterFileName = JSON.stringify(existingFileNames);
                const querySql = `UPDATE work SET is_submitted = $1, work = $2 ,sent_date =$3,verify = $4  WHERE id_user = $5 AND id = $6 RETURNING id_assignment`
                result = await db.query(querySql,[true,filterFileName,formatDate,false,id_user,id_work])
            }
            else{
                const querySql = `UPDATE work SET is_submitted = $1 , sent_date = $2,verify=$3 WHERE id_user = $4 AND id = $5 RETURNING id_assignment`
                result = await db.query(querySql,[true,formatDate,false,id_user,id_work])
            }
        }
...
exports.cancel_work = async (req, res) => {
    try {
        ...
        let querySql
        if(resultCheckType.rows[0].assignment_type === 'group'){
            querySql = `UPDATE work SET is_submitted = $1 ,sent_date = $2,verify = $3 FROM groups_member AS gm
                WHERE 
                    gm.id_group = work.id_group
                    AND gm.id_user = $4
                    AND work.id = $5`
            ...
            querySql = `UPDATE work SET is_submitted = $1,sent_date = $2,verify = $3 WHERE id_user = $4 AND id = $5 `
        }
...
exports.add_comment_work = async(req, res) => {
    try {
        const {message,id_work,date} = req.body
        console.log(message,id_work,date)
        ...
        const querySql = `INSERT INTO comment_work (message,date,id_work) VALUES ($1,$2,$3)`
        await db.query(querySql,[message,curentDate,id_work])
        return res.json({status: 'success'})
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: error.message });
...
exports.add_comment_work = async (req, res) => {
    try {
        ...
        const querySql = `SELECT * FROM comment_work WHERE id_work = $1 ORDER BY id DESC`
        const result = await db.query(querySql,[id_work])
        return res.json(result.rows)
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: error.message });
