...
const Detali_assignment = ({isLogin}) => {
    ...
    const handleFeedback = async() => {
        ...
        formdata.append('id_work',filteredAssignments.map((assignments) =>assignments.id))
    }
                        ...
                        {filteredAssignments.map((m,i) => (
                            ...
                            <td class="px-4 py-2 overflow-x-auto">
                                ...
                                {openFeed ? (
                                        <div className="flex items-center py-2">
                                            {assignmentType === 'group' ? (
                                                <textarea name={m.id_group} id="" className="border-2 text-black p-1" rows="5" cols="33" onChange={(e) => handleFeedbackChange(m.id_group, e.target.value)} >{m.feedback ? m.feedback : ''}</textarea>
                                            ):
                                            (
                                                <textarea name={m.id_user} id="" className="border-2 text-black p-1" rows="5" cols="33" onChange={(e) => handleFeedbackChange(m.id_user, e.target.value)} >{m.feedback ? m.feedback : ''}</textarea>
                                            )}
