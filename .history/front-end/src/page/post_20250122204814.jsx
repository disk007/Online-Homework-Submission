...
const DetailClassroom = ({isLogin}) =>{
    ...
    const addComment = async(id) => {
        ...
        const filteredvalueComment = valueComment[id]
        if(!isContentValid(filteredvalueComment)) {
            isValid = false
            setErrorComemt('comment is required.')
        }
        if(isValid){
            formdata.append('message',filteredvalueComment)
            const response = await axios.post('/add-comment',data,{
                            ...
                            
                            {
                                comment[data.id] ? (
                                    <>
                                        ...
                                        <div className="relative ml-3 top-2 cursor-pointer" onClick={()=>addComment(data.id)}><LuSendHorizonal className="w-5 md:w-6 md:h-6 h-5 text-sky-500 comment" /></div>
                                        {errorComemt && <div className="mt-1 text-sm text-red-500">{errorComemt}</div>}
                                    </>
                                )
                                :(
