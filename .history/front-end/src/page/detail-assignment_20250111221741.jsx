...
const Detali_assignment = ({isLogin}) => {
    ...
    const sortData = (field) => {
        ...
        setSortField(field); // อัปเดตฟิลด์ที่กำลังจัดเรียง
        setSortOrder(order); 
        const sortedData = [...detailAssignments].sort((a, b) => {
            if (field === 'name') {
                return order === 'asc' ? a.fname.localeCompare(b.fname) : b.fname.localeCompare(a.fname);
            } else if (field === 'state') {
                const valueA = a.is_submitted ? 1 : 0;
                        ...
                        <tr class="border-t-2 border-b-2 text-sm">
                            ...
                            <th class="px-4 py-2 text-center">
                                <div className="flex items-center justify-center">
                                    <span className="mx-1">Name</span> <div className="flex cursor-pointer" onClick={()=>{sortData('name')}}>{sortField === 'name' && (sortOrder === 'asc' ? <FaSortUp /> : <FaSortDown />)}</div>
                                </div>
                            </th>
                            <th class="px-4 py-2 text-center">
                                <div className="flex items-center justify-center">
                                    <span className="mx-1">State</span> <div className="flex cursor-pointer" onClick={()=>{sortData('state')}}> {sortField === 'state' && (sortOrder === 'asc' ? <FaSortUp /> : <FaSortDown />)}</div>
                                </div>
                            </th>
