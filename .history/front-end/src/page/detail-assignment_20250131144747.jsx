        ...
        <>
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                ...
                <div className="overflow-x-auto">
                    <table className="table-fixed md:w-full border-collapse border-spacing-0">
                        <thead>
                        ...
                        {filteredAssignments.map((m,i) => (
                            ...
                            <td className="px-4 py-2">
                                <div className="flex md:flex-row flex-col items-center justify-center text-sky-600 cursor-default" title={ m.is_submitted === true? formattedDate(m.sent_date): undefined}
>
                                    {m.is_submitted == true  && new Date(m.sent_date) < new Date(m.due_time) && 
                                    <div className="flex"><FaCheck className="mr-1" /> <span>Turned in</span></div>
                                    }
                                    {m.is_submitted == true  && new Date(m.sent_date) > new Date(m.due_time) &&
                                    <div className="flex"><FaCheck className="mr-1" /> <span>Turned in late</span></div>
                                    }
                                    {m.is_submitted == false  && 
                                    <div>
                                        <AiOutlineStop className="mr-1" /><span>Turned out</span> 
                                    </div>
                                    }
                                </div>
                            </td>
