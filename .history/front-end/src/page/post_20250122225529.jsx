...
const DetailClassroom = ({isLogin}) =>{
    ...
    console.log("dataComment "+dataComment.length)
    const addPost = async() => {
        let sizeFiles = 0
        let isValid = true
    ...
    const addComment = async(id) => {
        ...
        const filteredvalueComment = String(valueComment[id]|| '') valueComment[id] || '';
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                ...
                <div className="border-2 rounded-lg mt-5 lg:mx-24 md:mx-16 mx-10 shadow py-4">
                    ...
                    <div className="border-t-2 mt-2">
                        ...
                        <div className="text-gray-500 mx-4 my-3">{dataComment.filter((d) => d.id_post == data.id).length} comments</div>
                        {dataComment.length > 0 && dataComment.filter((d) => d.id_post == data.id).map((c,i) =>(
                            ...
                            
                            {
                                comment[data.id] ? (
                                    ...
                                    <div>
                                        ...
                                        <div className={`relative ml-3 top-2 ${!isContentValid(String(valueComment[data.id] || '')) ? 'text-gray-400': 'text-sky-500 cursor-pointer'}`} onClick={()=>addComment(data.id)}><LuSendHorizonal className="w-5 md:w-6 md:h-6 h-5  comment" /></div>
                                    </>
                                )
                                :(
