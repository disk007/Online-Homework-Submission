...
const DetailClassroom = ({isLogin}) =>{
    ...
    return(
        <>
            ...
            {
                openEditComment && (
                    <ModelEditComment open={openEditComment} onClose={()=>openEditComment(false)} id={selecteId} FetchAllComment={fetchComment}/>
                )
            }
            { selectFile && 
            (
                                        ...
                                        </div>
                                            {
                                                editDelComment[c.id] && (
                                                    <div className="absolute left-2 bg-white cursor-pointer comment-editDel">
                                                        <div className="text-xs border-t-2 border-x-2 p-1 hover:bg-gray-200" onClick={()=>{chooseDataEdit(c.id,c.message);setOpenEditComment(!openEditComment)}}>Edit</div>
                                                        <div className="text-xs border-2 p-1 hover:bg-gray-200">Delete</div>
                                                    </div>
                                                )
                                            }
