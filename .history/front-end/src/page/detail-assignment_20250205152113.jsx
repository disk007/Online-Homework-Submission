...
const Detali_assignment = ({isLogin}) => {
    ...
    useEffect(() => {
        const content = document.querySelector(".detail-assignment") // เลือก element ที่ต้องการ
    
        if (content) {
            if (openFile) {
                content.style.overflow = "hidden"; // ปิดการเลื่อน
            } else {
                content.style.overflow = ""; // เปิดการเลื่อนเมื่อปิด ModelFile
            }
        }
    
        return () => {
            if (content) {
                content.style.overflow = ""; // คืนค่าเดิมเมื่อ component ถูก unmount
            }
        };
    }, [openFile]);
    const handleExport = () => {
    ...
    return (
        <>
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 detail-assignment ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
