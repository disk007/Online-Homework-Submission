...
const Member = ({ isLogin }) => {
    ...
    const handleDelete = async () => {
        try {
            ...
            fetchMembers(); // รีเฟรชข้อมูลหลังจากลบ
        } catch (error) {
        ...
        <>
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}`}>
                <div className={`flex pl-5 md:text-lg md:font-medium bg-gray-100 border-b-2 py-3 sticky md:top-[67px] top-[59px] items-center`}>
                    <div className="lg:mx-1 mx-0 block lg:hidden" onClick={() => setSidebar(!sidebar)}>
                        <div className="p-1">
                            <GiHamburgerMenu className="h-5 w-5 text-black cursor-pointer" />
                        </div>
                    </div>
                    <div className="lg:mx-1 mx-0">
                        <div className="hidden lg:block bg-sky-600 rounded p-1">
                ...
                <div className="my-5 flex flex-col lg:mx-24 md:mx-16 mx-10">
                    ...
                    <div className="my-2 flex items-center">
                        <div className="border-2 p-2 rounded-full mx-3"><FaUser className="h-5 w-5" /></div>
                        <div>{teacher.name}</div>
                    </div>
                    <div className="mt-8 text-xl font-semibold mb-2 flex justify-between items-center">
                        <div>Student</div>
                        <div className="text-sm font-normal">{member.length} student</div>
                    </div>
                    <hr className="mb-2" />
                    <div className="my-3 flex items-center">
                        <div className="mx-3"><input type="checkbox" checked={selected.every(item => item)} onChange={toggleAllCheckboxes} className="transform scale-150" /></div>
                        ...
                        <div className="my-2 flex items-center" key={index}>
                            <div className="mx-3">
                                <input 
                                    ...
                                    onChange={() => handleCheckboxChange(index, data.id_classroom, data.id_user)} 
                            ...
                            <div className="cursor-pointer hover:bg-gray-200 p-1 rounded-full" onClick={() => handleCheckboxChange(index, data.id_classroom, data.id_user)}><MdDelete className="h-5 w-5" /></div>
                        </div>
                    ))}
                </div>
            </div>
