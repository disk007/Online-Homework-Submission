...
const Detali_assignment = ({isLogin}) => {
    ...
    const handleScoreChange = (index, value) => {
        const newScores = [...scores]; // คัดลอกอาร์เรย์เดิม
        newScores[index] = value; // อัปเดตค่าที่ index นั้น
        setScores(newScores); // เซ็ตค่ากลับไปที่ state
    };
    const fillAllScores = (value) => {
        setScores(scores.map(() => value)); // ตั้งค่าเดิมในทุกช่อง
    };
    ...
    const filteredAssignments = detailAssignments.filter((assignment) => {
      ...
      const [scores, setScores] = useState(Array(detailAssignments.length).fill(''));
      useEffect(() => {
                                    ...
                                    <div className="flex items-center justify-center">
                                        <div className="mr-1">
                                            <input type="text" name="" id="" 
                                                ...
                                                value={scores[0]} // ใช้คะแนนช่องแรกเป็นตัวอย่าง
                                                onChange={(e) => fillAllScores(e.target.value)} 
                                            />
                            ...
                            {filteredAssignments[0]?.score !== '' &&
                                <td className="px-4 py-2 text-center w-28 ">
                                    <div className="mr-1">
                                        <input type="text" name="" id="" 
                                            className="text-center w-8 border-2 font-normal text-sm"
                                            value={scores[i]} // ผูกค่ากับ state ของแต่ละช่อง
