...
const DetailClassroom = ({isLogin}) =>{
    ...
    const listAssignments = async() => {
        try {
            if(isLogin.role === 'teacher'){
                const response = await axios.get(`/page-post-assignments/${classroomId}`)
                const responseData = response.data
                setAssignment(responseData)
            }
            else if(isLogin.role === 'student'){
                const response = await axios.get(`page-post-work/${classroomId}/${isLogin.id}`)
                const responseData = response.data
                setAssignment(responseData)
            }
    ...
    const assignmentLink = (id) => {
        if(isLogin.role === 'teacher'){
            ...
            navigate(`/assignments/send-work/${id}`);
        }
    }
    return(
        <>
            ...
            {
                openEditComment && (
                    <ModelEditComment open={openEditComment} onClose={()=>setOpenEditComment(false)} id={selecteId} FetchAllComment={fetchComment}/>
                )
            }
            { selectFile && 
            (
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                ...
                <div key={index}>
                    ...
                    <>
                        <div className="border-2 rounded-lg mt-5 lg:mx-24 md:mx-16 mx-10 shadow py-4 cursor-pointer hover:bg-gray-100" onClick={()=>assignmentLink(isLogin.role === 'teacher' && data.id_assignment && isLogin.role === 'teacher' && data.id_assignment)}>
