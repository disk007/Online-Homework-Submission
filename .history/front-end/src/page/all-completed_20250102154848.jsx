...
const All_completed = ({isLogin}) => {
    ...
    const handleLinkClick = (workId,id_classroom) =>{
        navigate(`/assignments/send-work/${id_classroom}/${workId}/${isLogin.id}`);
    }
            ...
            {assignment.map((data, index) => {
                    ...
                    return (
                        <div className="w-full mb-5" key={index}>
                            ...
                            <div className="flex justify-center items-start border-2 py-2 px-4 mx-5 rounded-md shadow cursor-pointer" onClick={()=>handleLinkClick(data.id,data.id_classroom)}>
