...
const DetailClassroom = ({isLogin}) =>{
    ...
    return(
        <>
            ...
            {
                openEdit && (
                    <ModelEditPost open={openEdit} onClose={()=>setOpenEdit(false)} id={selectedEDit.id} data={selectedEDit.data} />
                )
            }
            { selectFile && 
            (
                ...
                <div className="border-2 rounded-lg mt-5 lg:mx-24 md:mx-16 mx-10 shadow py-4">
                    <div className="flex ">
                        ...
                        <div className="mx-2 relative">
                            <div className="cursor-pointer hover:bg-gray-200 p-1 rounded-full post-editDel" onClick={()=>toggleEditDel(data.id)}>
                                <BsThreeDotsVertical className="w-5 h-5  " />
                            </div>
                            {
                                editDel[data.id] && (
                                    <div className="absolute left-2 bg-white cursor-pointer">
                                        <div className="text-xs border-t-2 border-x-2 p-1 hover:bg-gray-200" onClick={()=>chooseDataEdit(data.id,data.message)}>Edit</div>
                                        <div className="text-xs border-2 p-1 hover:bg-gray-200">Delete</div>
                                    </div>
                                )
                            }
                            ...
                            <div key={i} className={`border-t-2`}>
                                <div className="flex mt-5">
                                    ...
                                    <div className="mx-1 grow  text-sm">
                                        <div >{c.name}</div>
                                        <div className="text-xs text-gray-500">{formattedDate(c.create_at)}</div>
                                    </div>
                                    <div className="mx-2 relative">
                                        <div className="cursor-pointer hover:bg-gray-200 p-1 rounded-full " onClick={()=>toggleEditDelComment(c.id)}>
                                        ...
                                        </div>
                                            {
                                                editDelComment[c.id] && (
                                                    <div className="absolute left-2 bg-white cursor-pointer">
                                                        <div className="text-xs border-t-2 border-x-2 p-1 hover:bg-gray-200">Edit</div>
                                                        <div className="text-xs border-2 p-1 hover:bg-gray-200">Delete</div>
                                                    </div>
                                                )
                                            }
                                ...
                                <div className={`list-inside ${data.message.includes("<ol>") || data.message.includes("<ul>") ? "pl-4" : "pl-2"}`} dangerouslySetInnerHTML={{ __html: c.message }} />
                                </div>
                            </div>
                        ))}
