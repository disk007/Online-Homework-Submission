                    ...
                    <table className="table-fixed md:w-full border-collapse border-spacing-0">
                        ...
                        {filteredAssignments.map((m,i) => (
                            ...
                            <td className="px-4 py-2">
                                <div className="flex items-center text-[10px] justify-center text-sky-600 cursor-default" title={ m.is_submitted === true? formattedDate(m.sent_date): undefined}
>
                                    {m.is_submitted == true  && new Date(m.sent_date) < new Date(m.due_time) && 
                                    <><FaCheck className="mr-1" /> Turned in</>
                                    }
                                    {m.is_submitted == true  && new Date(m.sent_date) > new Date(m.due_time) &&
                                    <><FaCheck className="mr-1" /> Turned in late</>
                                    }
                                    {m.is_submitted == false  && 
                                    <>
                                        <AiOutlineStop className="mr-1" /> Turned out
                                    </>
                                    }
                                </div>
                            </td>
