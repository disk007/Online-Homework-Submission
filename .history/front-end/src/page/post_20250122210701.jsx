...
const DetailClassroom = ({isLogin}) =>{
    ...
    const addComment = async(id) => {
        ...
        if(!isContentValid(filteredvalueComment)) {
            isValid = false
            // setErrorComment((prev) => ({ ...prev, [id]: 'comment is required.' }));
        }
        if(isValid){
                        ...
                        <div className={`flex md:mx-4 mx-3 mt-4 justify-center ${comment[data.id] ? "items-start" : "items-center"}`}>
                            
                            {
                                comment[data.id] ? (
                                    <>
                                    <div>
                                    dddddddddddddd{valueComment[data.id]}
                                    </div>
                                        ...
                                        <div className={`relative ml-3 top-2 cursor-pointer ${errorComment ? 'text-red-500' : 'text-sky-500'}`} onClick={()=>addComment(data.id)}><LuSendHorizonal className="w-5 md:w-6 md:h-6 h-5 text-sky-500 comment" /></div>
                                    </>
                                )
                                :(
