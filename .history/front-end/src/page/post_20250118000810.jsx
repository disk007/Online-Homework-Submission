...
const DetailClassroom = ({isLogin}) =>{
    ...
    const handleDelele = (fileName) => {
        setFileNames(prevFileNames => prevFileNames.filter(file => file !== fileName))
    }
    const handleCancle = () =>{
        setPost(!post)
        setValue('')
    ...
    return(
        <>
            ...
            <ModelEditAssignment assignmentId={409}  isLogin={isLogin} open={openEdit} OnClose={()=>setOpenEdit(false)} />
            <ModelImg open={imgModel} OnClose={()=>setImgModel(false)} name={nameImg} />
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                <button onClick={() =>setOpenEdit(!openEdit)}>hhhhh</button>
                        ...
                        <div className="my-4 flex  flex-col lg:mx-24 md:mx-16 mx-10 border-2 p-2 rounded-lg">
                                ...
                                {fileNames.length > 0 && (
                                    <div className="flex flex-wrap text-xs">
                                        {fileNames.map((name,index)=>(
                                            <div key={index} className="flex items-center mb-2 border-2 w-60 mr-2" title={name}>
                                                <button className="px-2 py-2 flex flex-1">
                                                    <div className=""><FaFileAlt className="w-4 h-4" /></div>
                                                    <div className="">
                                                        <span className="pl-1">{name.length > 20 ? name.substring(0, 20) + "..." : name}</span>
                                                    </div>
                                                </button>
                                                
                                                <div className="flex">
                                                    <button className="pr-1" onClick={()=>handleDelele(name)}><RxCross2 className="w-4 h-4" /></button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="mt-2 flex justify-between items-center">
                                <div className="border-2 inline-block p-1 rounded-full hover:bg-gray-300">
                                    <input type="file" className="hidden" id="file" multiple onChange={handleFileChange}  accept=".pdf,.doc,.docx,image/*"/>
                                    <label htmlFor="file" className="cursor-pointer"><MdOutlineFileUpload className="w-8 h-8" /></label>
                                </div>
                                <div><button className="hover:bg-sky-600 text-white bg-sky-500 py-1 transition ease-in-out delay-150 rounded px-7">Post</button></div>
                            </div>
                        </div>
                        ) : (
