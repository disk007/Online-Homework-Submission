                                ...
                                <div className={`text-[10px] h-32 ${m.work !== null ? 'h-20 overflow-y-auto' : 'h-auto'}`}>
                                    ...
                                    {m.work && JSON.parse(m.work).map((file,index) => (
                                        ...
                                        <div className="flex items-center justify-center mb-2 border-2" key={index} title={file}>
                                            <div className="px-2 py-2 flex flex-wrap cursor-pointer w-full" onClick={()=>{setOpenFile(!openFile);selectFileWork(file,m.id,m.assignment_type === 'group'?'group':null,m.assignment_type === 'group'? m.id_group : m.id_user)}}>
