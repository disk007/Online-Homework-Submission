...
const All_past_due = ({isLogin}) => {
    ...
    const upComming = async () => {
        try{
            const response = await axios.get(`/past-due/${isLogin.id}`)
                    ...
                    <div className="w-full mb-8" key={index}>
                        ...
                        <div className="flex justifly-center items-start border-2 py-2 px-4 mx-5 rounded-md shadow cursor-pointer">
                            ...
                            <div className="flex flex-col grow">
                                <div>{data.title}</div>
                                <div className={`${new Date() > new Date(data.due_time) ? 'text-red-500 ': 'text-gray-500'} text-sm`}>Due {formattedDate(data.due_time)}</div>
                                <div className="text-gray-500 text-sm">{data.name}</div>
                            </div>
                            <div className="flex justifly-center bg-green-100 md:p-2 p-1 items-center">
                                <AiOutlineStop className="md:h-4 md:w-4 text-green-500 hidden md:block" />
