    ...
    return(
        ...
        <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 md:py-5 py-2 bg-white  ${sidebar ? 'opacity-10 pointer-events-none' : ''}`}>
            {assignment.map((data,index)=>(
                // <div key={i} className=" border-2 rounded-lg mt-5 md:mx-16 mx-10 shadow py-4 cursor-pointer hover:bg-gray-100 text-sm" >
                //     <div className="flex-col mx-4 my-1 grow">
                //         <div>{data.title} </div>
                //         <div className="text-gray-500">Due {formattedDate(data.due_time)}</div>
                //     </div>
                // </div>
                <div className="w-full mb-8 h-" key={index}>
                    ...
                    <div className="flex justifly-center items-start border-2 py-2 px-4 mx-5 rounded-md shadow cursor-pointer">
                        <div className="bg-fuchsia-500 text-white p-2 flex items-center justify-center w-8 h-8 rounded-md text-sm mr-3">dddd{extractFirstChars(data.name)}</div>
