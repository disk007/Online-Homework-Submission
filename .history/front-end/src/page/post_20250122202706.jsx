...
const DetailClassroom = ({isLogin}) =>{
    ...
    const [errorComemt,setErrorComemt] = useState('')
    ...
    const handleFileCommentChange = (e) => {
        ...
        const newFiles = files.map((file) => {
            ...
            let count = 1;
            while (filecomment.some((f) => f.name === fileName)) {
                fileName = `${baseName}(${count})${extension}`;
                count++;
            }
            const mimeType = file.type
        ...
        setFilecomment((prevFiles) => [...prevFiles, ...newFiles]);
        e.target.value = '';
    };
    const handleDelete = (fileName) => {
        setFileNames(prevFileNames => prevFileNames.filter(file => file.name !== fileName))
    }
    const handleDeleteComment = (fileName) => {
        setFilecomment(prevFileNames => prevFileNames.filter(file => file.name !== fileName))
    }
    const handleCancle = () =>{
        setPost(!post)
        setValuePost('')
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                ...
                <div className="border-2 rounded-lg mt-5 lg:mx-24 md:mx-16 mx-10 shadow py-4">
                    ...
                    <div className="border-t-2 mt-2">
                        ...
                        <div className={`flex md:mx-4 mx-3 mt-4 justify-center ${comment[data.id] ? "items-start" : "items-center"}`}>
  ...
  comment[data.id] ? (
    <div className="w-full">
      ...
      <div className="mt-2 flex justify-between items-center comment">
        <div className="border-2 inline-block p-1 rounded-full hover:bg-gray-300">
          <input
            type="file"
            className="hidden"
            ...
            accept="application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/plain,image/png,image/jpeg"
          />
          <label htmlFor="file" className="cursor-pointer">
            <MdOutlineFileUpload className="w-8 h-8" />
          </label>
        </div>
        <button className="text-white hover:border-2 p-1 rounded-full comment">
          <LuSendHorizonal className="w-5 md:w-6 md:h-6 h-5 text-sky-500 comment" />
        </button>
      </div>
      {filecomment.length > 0 && (
        <div className="flex flex-wrap text-xs">
          {filecomment.map((f, index) => (
            <div
              key={index}
              className="flex items-center mb-2 border-2 w-60 mr-2"
              title={f.name}
            >
              <button
                className="px-2 py-2 flex flex-1"
    ...
    <div
      className="w-full cursor-pointer hover:text-sky-500 md:text-base text-sm"
