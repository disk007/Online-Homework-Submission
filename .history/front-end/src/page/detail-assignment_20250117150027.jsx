...
import ModelEditAssignmen from "../components/model-edit-assignment";
...
const Detali_assignment = ({isLogin}) => {
    ...
    return (
        <>
            ...
            <ModelEditAssignmen assignmentId={assignmentId}  isLogin={isLogin} open={openEdit} OnClose={()=>setOpenEdit(false)} />
            { selectFile && 
                (
                    <ModelFile
                        open={openFile}
                            ...
                            <td class="px-4 py-2 overflow-x-auto">
                                ...
                                {openFeed[m.id_user || m.id_group] ? (
                                        <div className="flex items-center py-2">
                                            {assignmentType === 'group' ? (
                                                <textarea name={m.id_group} id={m.id_group} className="border-2 text-black p-1 w-full md:w-1/3 lg:w-1/4" rows="5"  onChange={(e) => handleFeedbackChange(m.id_group, e.target.value)} value={feedbackData[m.id_group] || m.feedback || ""} />
                                            ):
                                            (
                                                <textarea name={m.id_user} id={m.id_user} className="border-2 text-black p-1 w-full md:w-1/3 lg:w-1/4" rows="5"  onChange={(e) => handleFeedbackChange(m.id_user, e.target.value)} value={feedbackData[m.id_user] || m.feedback || ""} />
                                            )}
