...
const Member = ({isLogin}) => {
    ...
    const isTeacher = isLogin?.role === 'teacher'; 
    ...
    if (!isLogin) {
        return <div>Loading...</div>; // แสดง Loading จนกว่าจะได้ข้อมูลจาก useAuth
    }
    if(isLogin === null){
    ...
    <div className="my-3 flex items-center">
        ...
        <div className="mx-3 grow">
            <button
                ...
                className={`${selectedIds.length === 0 ? 'pointer-events-none bg-gray-200' : 'bg-red-600'} text-white px-4 py-1 rounded-md`}
            >
                Delete
            </button>
        </div>
        <div className="cursor-pointer hover:bg-gray-200 p-1 rounded-full">
            <MdSortByAlpha className="h-7 w-7" />
        </div>
    </div>
)}
