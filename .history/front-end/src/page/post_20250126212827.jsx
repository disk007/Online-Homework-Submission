...
const DetailClassroom = ({isLogin}) =>{
    ...
    return(
        <>
            ...
            {
                openEditComment && (
                    <ModelEditPost open={openEdit} onClose={()=>setOpenEdit(false)} id={selecteId} FetchAllComment={fetchComment}/>
                )
            }
            { selectFile && 
            (
                            ...
                            </div>
                            {
                                editDel[data.id] && (
                                    <div className="absolute left-2 bg-white cursor-pointer post-editDel">
                                        <div className="text-xs border-t-2 border-x-2 p-1 hover:bg-gray-200" onClick={()=>{chooseDataEdit(data.id,data.message);setOpenEdit(!openEdit)}}>Edit</div>
                                        ...
                                        </div>
                                            {
                                                editDelComment[c.id] && (
                                                    <div className="absolute left-2 bg-white cursor-pointer comment-editDel">
                                                        <div className="text-xs border-t-2 border-x-2 p-1 hover:bg-gray-200" onClick={()=>{chooseDataEdit(c.id,data.message)}}>Edit</div>
                                                        <div className="text-xs border-2 p-1 hover:bg-gray-200">Delete</div>
                                                    </div>
                                                )
                                            }
