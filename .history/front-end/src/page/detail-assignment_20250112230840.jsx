...
const Detali_assignment = ({isLogin}) => {
    ...
    const handleInputChangeAll = (value) => {
        setScore(value); // อัปเดตค่า score เดี่ยว
        setScores((prev) => {
            console.log("Before update:", prev); // ดูค่าเดิมใน scores
            const updatedScores = Object.fromEntries(
                Object.keys(prev).map((key) => [key, value]) // อัปเดตทุก key
            );
            console.log("After update:", updatedScores); // ดูค่าที่จะถูกตั้งใหม่
