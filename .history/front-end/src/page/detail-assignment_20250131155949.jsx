...
const Detali_assignment = ({isLogin}) => {
    ...
    const handleExport = () => {
        if (!dataExcel.length) return; // ป้องกัน dataExcel ว่าง
    
        // ✅ ล้างชื่อ Sheet โดยลบอักขระต้องห้าม
        const sheetTitle = dataExcel[0].title.replace(/[\\/:?*[\]]/g, "");
    
        // สร้าง Header
        const header = [["Name", `Score = ${dataExcel[0].ascore}`]];
    
        // แปลงข้อมูลเป็น Array of Arrays (AOA)
        ...
        const fileName = `${sheetTitle}.xlsx`;
