                    ...
                    <table className="table-fixed w-full border-collapse border-spacing-0">
                        ...
                        {filteredAssignments.map((m,i) => (
                            ...
                            <td className="px-4 py-2 ">
                                <div className="flex items-center justify-center text-sky-600" title={m.due_time ? formattedDate(m.due_time):''}>
                                    {m.is_submitted == true  && new Date(m.sent_date) < new Date(m.due_time) && 
                                    <div><FaCheck className="mr-2" /> Turned in</div>
                                    }
                                    {m.is_submitted == true  && new Date(m.sent_date) > new Date(m.due_time) &&
                                    <div><FaCheck className="mr-2" /> Turned in late</div>
                                    }
