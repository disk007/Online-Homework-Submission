    ...
    return(
        <>
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                ...
                <div key={index}>
                    {"title" in item ? ( // ถ้าเป็น assignment
                    <>
                        <h3>{item.title}</h3>
                        <p>Due: {new Date(item.create_at).toLocaleString()}</p>
                        <p>Status: {item.true_count}/{item.all_count}</p>
                    </>
                    ) : ( // ถ้าเป็น post
                    <>
                        <h3>{item.name} ({item.role}) post</h3>
                        <p dangerouslySetInnerHTML={{ __html: item.message }}></p>
                        <p>Files: {item.file}</p>
                        <p>Created at: {new Date(item.create_at).toLocaleString()}</p>
                    </>
                    )}
                </div>
                ))}
