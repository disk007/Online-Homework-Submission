...
const Detali_assignment = ({isLogin}) => {
    ...
    const selectFileWork = async (n,workId,type,id) => {
        ...
        try {
            ...
            let response
            if(type === null){
                response = await axios.get(`/assignments/${assignmentId}/${workId}/${isLogin.id}/${encodedFileName}`, { 
                    responseType: 'arraybuffer' // ต้องตั้ง responseType เพื่อให้สามารถตรวจสอบ Content-Type ได้
                });
            }
            else{
                response = await axios.get(`/assignments/${assignmentId}/${workId}/${type}/${encodedFileName}`, { 
                    responseType: 'arraybuffer' // ต้องตั้ง responseType เพื่อให้สามารถตรวจสอบ Content-Type ได้
                });
            }
                                ...
                                <div className={`text-xs h-32 ${m.work !== null ? 'h-20 overflow-y-auto' : 'h-auto'}`}>
                                    ...
                                    {m.work && JSON.parse(m.work).map((file) => (
                                        ...
                                        <div className="flex items-center justify-center mb-2 border-2 " title={file}>
                                            <div className="px-2 py-2 flex flex-1 cursor-pointer" onClick={()=>{setOpenFile(!openFile);selectFileWork(file,data.id,data.assignment_type === 'group'?'group':null,data.assignment_type === 'group'? data.id_group : data.id_user)}}>
