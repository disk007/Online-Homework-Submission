...
const Member = ({isLogin}) => {
    ...
    const handleCheckboxChange = (index, id_classroom, id_user) => {
        ...
        // Check if the item is selected and update the selected IDs list accordingly
        if (newSelected[index]) {
            newSelectedIds.push({ id_classroom, id_user });
        } else {
            const filteredIds = newSelectedIds.filter(
                item => item.id_classroom !== id_classroom || item.id_user !== id_user
            );
    ...
    const handleDelete = () => {
        toggleAllCheckboxes(); // This will check all checkboxes when delete is clicked
    };
                        ...
                        <div className="my-2 flex items-center" key={index}>
                            <div className="mx-3">
                                <input 
                                    ...
                                    onChange={() => {
                                        handleCheckboxChange(index,data.id_classroom,data.id_user);
                                    }} 
