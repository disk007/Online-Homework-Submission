        ...
        <>
            ...
            <div className={`ml-[6rem] md:ml-[8rem] lg:ml-[26rem] mb-4 ${sidebar ? 'opacity-10 pointer-events-none' : ''}` }>
                ...
                <div className="overflow-x-auto">
                    <table className="table-fixed w-[700px] md:w-full border-collapse border-spacing-0">
                        <thead>
...
>
                                    ...
                                    {m.is_submitted == true  && new Date(m.sent_date) > new Date(m.due_time) &&
                                    <><FaCheck className="mr-2" /> Turned in late</>
                                    }
                            ...
                            <td className="px-4 py-2 w-full">
                                <div className={`text-[10px] w-full h-32 ${m.work !== null ? 'h-20 overflow-y-auto' : 'h-auto'}`}>
                                    {m.work === null && <div className="text-sky-600 text-center w-full">None</div>}
                                    {m.work && JSON.parse(m.work).map((file,index) => (
                                        
                                        <div className="flex items-center justify-center mb-2 w-full border-2 " key={index} title={file}>
                                            <div className="px-2 py-2 flex flex-1 cursor-pointer w-full" onClick={()=>{setOpenFile(!openFile);selectFileWork(file,m.id,m.assignment_type === 'group'?'group':null,m.assignment_type === 'group'? m.id_group : m.id_user)}}>
