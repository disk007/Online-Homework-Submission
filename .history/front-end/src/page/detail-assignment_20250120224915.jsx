...
const Detali_assignment = ({isLogin}) => {
    ...
    const handleDeleteAssignment = async() => {
        try {
            ...
            data.append('assignmentId',JSON.stringify(assignmentId))
            data.append('assignment_type',JSON.stringify(assignmentType))
            const response = await axios.post('/delete-assignment',assignmentId,{
                headers: {'Content-Type': 'application/json'}
            })
            const responseData = response.data
            if (responseData.status ==='success') {
