...
const Detali_assignment = ({isLogin}) => {
    ...
    const handleFeedbackChange = (id,value) => {
        setDetailAssignments((prevAssignments) =>
            prevAssignments.map((assignment) =>
                assignment.id_grop === id
                        ...
                        {filteredAssignments.map((m,i) => (
                            ...
                            <td class="px-4 py-2 overflow-x-auto">
                                ...
                                {openFeed[m.id_user || m.id_group] ? (
                                        <div className="flex items-center py-2">
                                            {assignmentType === 'group' ? (
                                                <textarea name={m.id_group} id={m.id_group} className="border-2 text-black p-1" rows="5" cols="33" onChange={(e) => handleFeedbackChange(m.id_group, e.target.value)} value={m.feedback || ""} />
                                            ):
                                            (
                                                <textarea name={m.id_user} id={m.id_user} className="border-2 text-black p-1" rows="5" cols="33" onChange={(e) => handleFeedbackChange(m.id_user, e.target.value)} value={m.feedback || ""} />
                                            )}
