...
const All_completed = ({isLogin}) => {
    ...
    const filterByMonth = (data) => {
        const result = data.reduce((acc, current) => {
          const currentDate = new Date(current.due_time);
          const monthYear = `${currentDate.getFullYear()}-${currentDate.getMonth()}`; // e.g., "2024-11"
    
          if (!acc[monthYear]) {
            acc[monthYear] = current; // Keep the first entry for each month
          }
          return acc;
        }, {});
    
        return Object.values(result); // Convert the result back to an array
      };
    
    const filteredAssignments = filterByMonth(assignment);
    if(!isLogin){
        ...
        <>
            ...
            <div className="flex justify-center md:ml-32 ml-[6rem] md:py-5 py-2 bg-white items-center flex-col">
                ...
                {filteredAssignments.map((data,index) => (
                    <div className="w-full mb-8" key={index}>
                        ...
                        <div className="flex justifly-center items-start border-2 py-2 px-4 mx-5 rounded-md shadow cursor-pointer">
                            ...
                            <div className="flex justifly-center bg-green-100 md:p-2 p-1 items-center">
                ...
                {/* <div className="w-full mb-8">
                    <div className="mx-5 mb-3">Jul 28th Today</div>
                    <div className="flex justifly-center items-start border-2 py-2 px-4 mx-5 rounded-md shadow cursor-pointer">
                        <div className="bg-fuchsia-500 text-white p-2 flex items-center justify-center w-8 h-8 rounded-md text-sm mr-3">ws</div>
                        <div className="flex flex-col grow">
                            <div>ใบงานที่ 5</div>
                            <div className="text-gray-500 text-sm">Submitted at 10.59 PM</div>
                            <div className="text-gray-500 text-sm">Python Beginner </div>
                        </div>
                        <div className="flex justifly-center bg-gray-100 md:p-2 p-1">
                            <AiOutlineStop className="h-4 w-4 text-gray-500 hidden block" />
                            <div className="md:text-sm text-xs text-gray-600">Not turned in</div>
                        </div>
                    </div>
                </div> */}
