            ...
            (
                <>
                    <div className={`border-b-2 ml-[6rem] md:ml-[8rem] lg:ml-[26rem] flex py-3  items-center bg-gray-100 text-xs md:text-base ${sidebar ? 'opacity-10 pointer-events-none' : ''}`}>
                        ...
                        <div className="md:px-4 px-3"><Link to={'/detail-classroom/up-comming'} className={`${activeLink === `/detail-classroom/up-comming/${classroomId}` ? 'font-medium' : ''}`}>Upcomming</Link><div className={`mt-0.5 h-0.5 transition ease-in-out delay-100 ${activeLink === '/detail-classroom/up-comming' ? ' w-full bg-sky-600 rounded-sm' : ''}`}></div></div>
                        <div className="md:px-4 px-3"><Link to={'/detail-classroom/past-due'} className={`${activeLink === `/detail-classroom/past-due/${classroomId}` ? 'font-medium' : ''}`}>Past due</Link><div className={`mt-0.5 h-0.5 transition ease-in-out delay-100 ${activeLink === '/detail-classroom/past-due' ? '  w-full bg-sky-600 rounded-sm' : ''}`}></div></div>
                        <div className="md:px-4 px-3"><Link to={'/detail-classroom/completed'} className={`${activeLink === `/detail-classroom/completed/${classroomId}` ? 'font-medium' : ''}`}>Completed</Link><div className={`mt-0.5 h-0.5 transition ease-in-out delay-100 ${activeLink === '/detail-classroom/completed' ? '  w-full bg-sky-600 rounded-sm' : ''}`}></div></div>
                    </div>
                </>   
