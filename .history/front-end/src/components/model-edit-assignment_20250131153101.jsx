...
const ModelEditAssignment = ({assignmentId,isLogin,open,OnClose}) => {
    ...
    const fetchSizesFile = async() => {
        try {
            if(dataAssignment[0].length > 0){
                const response = await axios.get(`/size-files-assignments/${assignmentId}/${dataAssignment[0].id}`)
                const responseData = response.data
    ...
    console.log("dataAssignment.length",dataAssignment[0].id.length)
    useEffect(()=>{
        fetchSizesFile()
