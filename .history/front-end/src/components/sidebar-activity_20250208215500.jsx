...
const SidebarActivity = ({sidebar,setSidebar,isLogin,setWorkId}) =>{
    ...
    useEffect(() => {
        socket.emit("joinRoom", isLogin.id); // เข้าร่วมห้อง
        console.log("Joined room:", isLogin.id);
        socket.on("activity-student", (data) => {
            console.log("Received update:", data);
            setActivitiesGroup(data);
        });

        return () => {
            socket.off("activity-student");
        };
    }, []);
    console.log("activitiesGroup ",activitiesGroup)
