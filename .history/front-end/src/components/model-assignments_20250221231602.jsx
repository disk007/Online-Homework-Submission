...
const ModelAssignmen = ({isLogin,open,OnClose,listAssignments}) => {
    const handleFetchData = () => {
        listAssignments(); // เรียก fetchPost จาก props
    };
    ...
    const [groupStudent,setGroupStudent] = useState(false)
    const [dataIndividual, setDataIndividual] = useState([])
    const [dataGroup, setDataGroup] = useState([])
    const [menuStudy, setMenuStudy] = useState(false)
    const [selectStudy, setSelectStudy] = useState([])
    const { classroomId } = useParams()
    const [CreateClassroom,setCreateClassroom] = useState([])
    const [ownStudy,setOwnStudy] = useState([])
    const [stateIndividual,setStateIndividual] = useState(true)
    const [stateGroup,setStateGroup] = useState(true)
    ...
    useEffect(() => {
        ...
        if(typeWork == null){
            ...
            displayText = `${dataIndividual.length} students`;
        }
        handleWork(displayText);
    }, [dataIndividual]);
    ...
    useEffect(() => {
        ...
        if(typeWork == null){
            ...
            displayText = `${dataGroup.length} groups`;
        }
        handleWork(displayText);
    }, [dataGroup]);
    
    const handleGroupStudy = (id) => {
        setSelectStudy(prev => {
            const newSelectStudy = prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id];
            if (newSelectStudy.length > 1) {
                setTypeWork("All students");
            } 
            return newSelectStudy;
        })
    }
    ...
    useEffect(()=>{
        const handleClickOutside = (e) =>{
            if(!e.target.closest(".group-study") && menuStudy && !e.target.closest(".toggle-study")){
                setMenuStudy(false)
            }
        }
    ...
    useEffect(()=>{
        const handleClickOutside = (e) =>{
            if(!e.target.closest(".type-work") && openWork && !e.target.closest(".toggle-type-work")){
                setOpenWork(false)
            }
        }
        document.addEventListener("mousedown", handleClickOutside)
    ...
    const fetchAddRoom = async () => {
        try{
            const response = await axios.get(`/add-classroom/${isLogin?.id}`);
            const responseData = response.data; 
            const newClassroom = responseData.filter(item => item.id != classroomId);
            setCreateClassroom(newClassroom);
        }
        catch (error) {
    ...
    const fetchRoom = async () => {
        try{
            ...
            const responseData = response.data; 
            setOwnStudy(responseData);
            setSelectStudy([responseData.id])
        }
        catch (error) {
            console.error('Error :', error);
        }
    ...
    const propIndividual = {
        ...
        setDataIndividual: setDataIndividual,
        selectedStudents : selectedStudents,
        setSelectedStudents:setSelectedStudents,
        saveSelectedStudents:saveSelectedStudents,
        setSaveSelectedStudents:setSaveSelectedStudents
    }
    return(
        <>
            <ToastContainer containerId="add-assignment" />
            ...
            <div className={`fixed inset-0 flex justify-center items-center  ${open ? "visible bg-black/20 z-40" : "invisible"}`}>
                <div className="bg-white rounded-md w-[600px] max-h-screen overflow-y-auto ">
                    ...
                    <div className="px-4 py-2">
                        <div className="mb-3">
                            <div>Title :</div>
                            <input type="text" className="border-2 w-full py-2 px-2 mt-1" name="title" value={formData.title} onChange={handleChange}  />
                        ...
                        <div>
                            <div className="mt-2 mb-3">
                                <div>Group study {selectStudy.length}</div>
                                <div className="w-full relative">
                                    <div className="border-2 py-2 px-2 hover:bg-gray-200 cursor-pointer flex items-center justify-between toggle-study" onClick={()=>setMenuStudy(!menuStudy)}>
                                    {selectStudy.length === 1 ? (
                                        <div>{ownStudy.name}</div>
                                        ) : (
                                        <div>{selectStudy.length} study groups</div>
                                    )}
                                        <SiGoogleclassroom/>
                                    </div>
                                    {menuStudy &&(
                                        <>
                                            {ownStudy &&(
                                                <ul className="absolute left-0 w-full border-2 bg-white z-10 group-study overflow-y-auto h-32">
                                               
                                                    <li  className="list-none px-2 py-2 cursor-pointer flex items-center pointer-events-none" onClick={()=>handleGroupStudy(ownStudy.id)} >
                                                        <input 
                                                            type="checkbox" 
                                                            className="w-4 h-4 mr-1 "
                                                            checked={selectStudy[0] === ownStudy.id}
                                                            name={ownStudy.name}
                                                        />
                                                        <div className="mx-2 bg-blue-600 text-white py-1 px-3 rounded text-sm w-12 text-center">
                                                            {extractFirstChars(ownStudy.name)}
                                                        </div>
                                                        <span className="text-gray-800">{ownStudy.name}</span>
                                                    </li>
                                                
                                                {CreateClassroom.map((data)=>(
                                                    <li key={data.id} className="px-2 py-2 hover:bg-gray-200 cursor-pointer flex items-center" onClick={()=>handleGroupStudy(data.id)} >
                                                        <input 
                                                            type="checkbox" 
                                                            className="w-4 h-4 mr-1"
                                                            checked={selectStudy.includes(data.id)}
                                                            name={data.name}
                                                        />
                                                        <div className="mx-2 bg-blue-600 text-white py-1 px-3 rounded text-sm w-12 text-center">
                        ...
                        <div className="flex items-end">
                            ...
                            <div className={`w-full ml-2 relative `}>
                                <div className="border-2 py-2 px-2 hover:bg-gray-200 cursor-pointer flex items-center justify-between toggle-type-work" onClick={()=>setOpenWork(!openWork)}>{typeWork}<FaUserPlus/></div>
                                {openWork  &&(
                                    <>
                                    <ul className="absolute left-0 w-full border-2 bg-white z-10 type-work list-none">
                                        { selectStudy && (
                                            <li
                                            className="px-2 py-2 hover:bg-gray-200 cursor-pointer"
                                            onClick={() => handleWork('All students')}
                                            >
                                            All students
                                            </li>
                                        )}
                                        ...
                                        { selectStudy.length === 1 && (
                                            <>
                                                <li
                                                    className="px-2 py-2 hover:bg-gray-200 cursor-pointer"
                                                    onClick={() => {
                                                        handleWork(`${dataIndividual.length} students`);
                                                ...
                                                <li
                                                    className="px-2 py-2 hover:bg-gray-200 cursor-pointer"
                                                    onClick={() => {
                                                        handleWork(`${dataGroup.length} groups`);
                                 ...
                                 <div className={`h-2 ${errors.typeWork && "text-red-500 text-xs"} `}>{errors.typeWork}</div>
                            </div>
                        ...
                        <div className="flex justify-end">
                            ...
                            <div className="ml-2"><button className="border-2 py-2 px-8 w-full mt-3 text-white border-sky-500 hover:border-sky-600 bg-sky-500 hover:bg-sky-600 transition ease-in-out delay-150" onClick={handleAssignment}>Create</button></div>
