...
const Work_sheet = ({isLogin,workId,sidebar,setSidebar}) =>{
    ...
    const handleFileChange = (e) => {
        ...
        const filteredData = myWork.filter(f => f.id == workId);
        let existingFileNames
if (filteredData.length > 0 && filteredData[0].work) {
   existingFileNames = JSON.parse(filteredData[0].work);
    console.log(existingFileNames); // ตรวจสอบผลลัพธ์
}
        // const existingFileNames = myWork.filter(f => f.id == workId).map(data => (JSON.parse(data.work)))
        const newFiles = files.map((file) => {
            ...
            while (fileNames.some((f) => f.name === fileName) || existingFileNames.in)) {
