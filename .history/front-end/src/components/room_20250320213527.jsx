...
const Room = ({data}) =>{
    ...
    return(
        <>  
            ...
            {data.role === 'admin' && 
                <div className="flex justify-end mx-2 mt-1">
                    <input
                        ...
                        id="search"
                        placeholder="Search ..."
                        className="border-2 px-2 py-2 text-sm rounded-sm w-64"
                        onChange={(e) => {
            ...
            <div className="md:pl-[10.5rem] pb-5 pt-1 pl-[8.5rem]">
                <div className="flex flex-wrap overflow-y-auto pb-5">
                    {
                        data.role !== 'admin' && dataRoom.map((d)=>(
                        <div className="border-2 md:w-80 w-64 h-48 m:h-60 rounded-md shadow-md m-5 hover:bg-gray-100 transition ease-in-out delay-150 relative cursor-pointer" onClick={() => handleLinkClick(d.id)} key={d.id}>
                                ...
                                </div>
                                {
                                    editDel[d.id] && (
                                        <div className="text-xs block fixed z-[27] border-2 min-w-12 absolute -right-6 -bottom-8 bg-white post-editDel">
                                            ...
                                            <div className="px-2 cursor-pointer hover:bg-gray-100" onClick={(e)=>selectDataDel(e,d.id,d.name)}>Delete</div>
                                        </div>
                                    )
                                }
                        </div>
                ...
                openDel && (
                    <div className="fixed inset-0 z-[51] flex justify-center items-center bg-black/20">
                        <div className="bg-white rounded-md p-4 w-[30rem] ">
                            ...
                            <div className='mt-5'>Do you want to delete classroom ?</div>
                            <div className="flex justify-end mb-1 mt-5">
                                <button className=" px-7 py-2  text-gray-400 hover:text-gray-500 border-2 transition ease-in-out delay-150 mr-1" onClick={()=>{setOpenDel(!openDel)}}>Cancel</button>
