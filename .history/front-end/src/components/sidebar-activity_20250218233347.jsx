...
const SidebarActivity = ({sidebar,setSidebar,isLogin,setWorkId}) =>{
    ...
    const update_status_teacher = async(id_assignment,id_classroom) => {
        try {
            const formData = new FormData()
            formData.append('id_assignment', id_assignment)
            formData.append('id_classroom', id_classroom)
            const response = await axios.post('/update-status-teacher',formData,{
                headers: {'Content-Type': 'application/json'}
            })
            const responseData = response.data
            if(responseData.status ==='success') {
                LinkPageAssignment(id_assignment,id_classroom)
            }
        } catch (error) {
            console.error(error)
        }
            ...
            {isLogin.role === 'student' && activities.map((data,id)=> (
                <div className={`p-3 flex justify-between text-sm border-b-2 border-gray-300 cursor-pointer hover:bg-gray-100 ${data.activity == 'No' || data.activity == 'Initial' ? 'bg-gray-200': ''}  ${selectedId == data.id ? 'bg-gray-200' : ''}`} key={id} onClick={()=>handleWorkId(data.id,data.assignment_type)}>
                        ...
                        <div className="mx-1 ">
                            ...
                            <span className="line-clamp-1 text-gray-500">{formattedDate(data.due_time)} {activities.length}</span>
                            <span className="line-clamp-2 text-xs">{data.name}</span>
                        </div>
                </div>
            ))}
            {isLogin.role === 'teacher' && activities.map((data,id)=> (
                <div className={`p-3 flex text-sm border-b-2 border-gray-300 cursor-pointer hover:bg-gray-100 ${data.activity == 'NoT' || data.activity == 'InitialT' ? 'bg-gray-200': ''}`} key={id} onClick={()=>update_status_teacher(data.id_assignment,data.id_classroom)}>
                        <div className="mx-1"><div className="bg-sky-600 rounded p-2"><FaBook className="h-9 w-8 text-white" /></div></div>
                        <div className="mx-1 ">
                            <span className="line-clamp-2"><span className="text-red-700 font-medium">{data.submissioncount}</span>{" "}{data.assignment_type !== 'group' ? 'students' : 'groups'}  Added a task</span>
...

        {
            sidebar &&(
                <div className={`md:ml-32 ml-[6rem] bg-white border-r-2 w-72 h-screen fixed bg-gray-50 lg:hidden block overflow-y-auto z-40 sidebar `}>
                    ...
                    {isLogin.role === 'teacher' && activities.map((data,id)=> (
                        <div className={`p-3 flex text-sm border-b-2  cursor-pointer hover:bg-gray-100 ${selectedId == data.id ? 'bg-gray-200' : ''}`} key={id} onClick={()=>LinkPageAssignment(data.id_assignment,data.id_classroom)}>
                                <div className="mx-1"><div className="bg-sky-600 rounded p-2"><FaBook className="h-9 w-8 text-white" /></div></div>
                                <div className="mx-1 ">
                                    <span className="line-clamp-2">{data.submissioncount + ' ' + (data.assignment_type !== 'group' ? 'students' : 'groups') + ' Added a task'}</span>
                                    <span className="line-clamp-1 text-gray-500">{data.title}</span>
                                    <span className="line-clamp-2 text-xs">{data.name}</span>
                                </div>
                        </div>
