            ...
            <div className="md:w-32 border-x-2 h-screen w-24 fixed">
                <ul className="">
                    <li className={`py-4  ${activeLink.includes('/activity') ? "border-l-4 border-sky-600 text-sky-600 font-medium" : ""} transition ease-in-out delay-150 hover:bg-gray-100 hover:text-sky-600"`}>
                        <Link to={'/activity'} className="flex flex-col items-center relative">
                            {countActivity !== '0' &&
                                <div className="absolute right-5 p-1 w-8 h-8 flex items-center justify-center rounded-full bg-red-500 text-white text-lg">
                                    {countActivity}
                                </div>
                            }
