...
const Room = ({data}) =>{
    ...
    const selectDataEdit = (id,data) => {
        setDataEdit({id:id,name:data.name})
    ...
    return(
        <>  
            {openEdit &&
                <ModelEditRoom open={openEdit} onClose={()=>setOpenEdit(false)} />
            }
                                ...
                                </div>
                                {
                                    editDel[d.id] && (
                                        <div className="text-xs block fixed z-[27] border-2 min-w-12 absolute -right-6 -bottom-8 bg-white post-editDel">
                                            <div className="border-b-2 px-2 cursor-pointer hover:bg-gray-100" onClick={()=>selectDataEdit(d.id,d.name)}>Edit</div>
