    ...
    return (
        ...
        <div className=" md:ml-32 ml-[6rem] bg-white border-r-2 md:w-72 h-screen fixed hidden lg:block bg-gray-50 overflow-y-auto " >
            ...
            {isLogin.role === 'student' && activities.map((data,id)=> (
                <div className={`p-3 flex justify-between text-sm border-b-2  cursor-pointer hover:bg-gray-100 ${data.activity == 'No' ? 'bg-gray-200': ''}  ${selectedId == data.id ? 'bg-gray-100' : ''}`} key={id} onClick={()=>handleWorkId(data.id,data.assignment_type)}>
