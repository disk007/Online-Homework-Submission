...
const Work_sheet = ({isLogin,workId,sidebar,setSidebar}) =>{
    ...
    const fetchSizesFile = async() => {
        try {
            ...
            const response = await axios.get(`/size-files-work/${dataId[0].id_assignment}/${workId}/${dataId[0].assignment_type === 'group' ? dataId[0].id_group : dataId[0].id_user}`)
            const responseData = response.data
    ...
    console.log("work.filter(f => f.id == workId)", myWork.filter(f => f.id == workId))
    useEffect(() => {
        fetchSizesFile();
        setFileNames([]);
    }, [workId]);
