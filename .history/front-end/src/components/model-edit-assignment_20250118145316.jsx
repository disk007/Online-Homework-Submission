...
const ModelEditAssignment = ({assignmentId,isLogin,open,OnClose}) => {
    ...
    console.log('formData.colses_time',dataAssignment.colses_time)
    useEffect(() =>{
        fetchdataAssignment()
    },[])
    ...
    const handleQuillChange = (value) => {
        if (formData.instructions !== value) {
            ...
            setDataAssignment({...dataAssignment,instructions: value})
        }
    };
    ...
    const handleEditAssignment = async (e) => {
        ...
        if (formData.score !== '' && !/^[0-9]+$/.test(formData.score)) {
            isValid = false;
            validation.score = "Points must be a positive number.";
        }
        // if(new Date(dataAssignment.due_time) < checkDate){
        //     isValid = false;
        ...
        if (
            ...
            (checkDate.getFullYear() === dueDate.getFullYear() && checkDate.getMonth() === dueDate.getMonth() && checkDate.getDate() === dueDate.getDate() && checkDate.getHours() > dueDate.getHours()) || // วันเดียวกัน แต่ชั่วโมงปัจจุบัน > ชั่วโมงที่กำหนด
            (checkDate.getFullYear() === dueDate.getFullYear() && checkDate.getMonth() === dueDate.getMonth() && checkDate.getDate() === dueDate.getDate() && checkDate.getHours() === dueDate.getHours() && checkDate.getMinutes() > dueDate.getMinutes()) || // ชั่วโมงเดียวกัน แต่นาทีปัจจุบัน > นาทีที่กำหนด
            (checkDate.getFullYear() === dueDate.getFullYear() && checkDate.getMonth() === dueDate.getMonth() && checkDate.getDate() === dueDate.getDate() && checkDate.getHours() === dueDate.getHours() && checkDate.getMinutes() === dueDate.getMinutes() && checkDate.getSeconds() > dueDate.getSeconds()) // นาทีเดียวกัน แต่วินาทีปัจจุบัน > วินาทีที่กำหนด
        ) {
            isValid = false;
            validation.dueDate = "Due date must not be before the current date and time.";
        }
    ...
    console.log("due_time: "+new Date(dataAssignment.due_time)+'current time '+new Date());
    return(
        <>
            <ToastContainer />
            { selectFile && 
                        ...
                        {/* <div className="mb-3">
                            ...
                            <div className={`flex ${isChecked ?'text-black': 'text-gray-400'}`}>
                                <div className="w-full mr-1">
                                    <div>Close date:</div>
                                    <Flatpickr
                                        value={new Date(dataAssignment.colses_time)}
                                        className={`border-2 w-full py-2 px-2 mt-1 `}
                                        onChange={(selectedDates) => {
