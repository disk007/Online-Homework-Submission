...
import ModelFileBackEnd from "./model-file-back-end";
...
const Work_sheet = ({isLogin,workId,sidebar,setSidebar}) =>{
    ...
    const submitComment = async(e) => {
        ...
        if(!value.trim()){
            ...
            const response = await axios.post('/comment-work',data,{
    ...
    const selectFileName = async (n,id_assignment) => {
        ...
        try {
            ...
            handleSelectFile(blobWithName,mimeType)
            // setFileNameBackEnd({
            //     url: blob,
            //     type: mimeType,
            // });
    ...
    const selectFileWork = async (n,id_assignment) => {
        // setFileName(n)
        try {
            const encodedFileName = encodeURIComponent(n);
            // setFileName(pathFile)
            const response = await axios.get(`/assignments/${id_assignment}/${workId}/${isLogin.id}/${encodedFileName}`, { 
              responseType: 'arraybuffer' // ต้องตั้ง responseType เพื่อให้สามารถตรวจสอบ Content-Type ได้
            });
    ...
    // const xxx = myWork.filter(f => f.id == workId).map(data => (JSON.parse(data.work)))
                        ...
                        {myWork.filter(f => f.id == workId).map((data,i) => (
                            <div className="text-sm text-gray-500 xl:basis-1/3">
                                <div>
                                    ...
                                    {data.ascore !== '' && (
                                        <div className="mt-2">
                                            <div className="ml-1">Score : <span className="text-red-500">{data.wscore == null ?'...':data.wscore}{' / '+data.ascore}</span></div>
                                        </div>
                                    )}
                                    {data.verify === true && (
                                        <div className="mt-2">
                        ...
                        <div className="text-center lg:text-xl text-lg my-4">
                            ...
                            {data.work && JSON.parse(data.work).map((file) => (
                                ...
                                <div className="flex items-center mb-2 border-2" title={file}>
                                    <div className="px-2 py-2 flex flex-1 cursor-pointer" onClick={()=>{selectFileWork(file,data.id_assignment,data.assignment_type === 'group' ? );setOpen(!open)}}>
                                        <div className=""><FaFileAlt className="w-4 h-4" /></div>
                                        <div className="">
