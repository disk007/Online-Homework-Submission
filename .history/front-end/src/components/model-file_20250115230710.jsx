      ...
      if (file && type === "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
        ...
        reader.onload = (e) => {
          ...
          docx.renderAsync(arrayBuffer, node)
            .then(() => {
              const content = node.querySelector('.docx-content'); // สมมติว่า class นี้คือเนื้อหาที่เรนเดอร์
    if (content) {
      content.style.width = '100%'; // ทำให้เนื้อหามีความกว้างเต็มที่
      content.style.height = 'auto'; // ปรับความสูงให้ยืดหยุ่น
    }
          ...
          <div className="flex-grow flex justify-center my-1">
            <div className="p-2 border-2 bg-white">
              {type === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ? (
                <div className="docx-content" ref={panelSectionRef} 
                />
              ) : type === "application/pdf" || type === 'text/plain; charset=UTF-8' ? (
                <object
                  className="w-[600px] md:w-[700px] h-screen"
                  data={fileUrl}
