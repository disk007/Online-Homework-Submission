...
const ModelEditPost = ({open,onClose,id}) => {
    ...
    const fetchPost = async() => {
        try{
            ...
            setValuePost(responseData.message)
        } catch (error) {
            console.error(error)
        }
    }
    const fetchSizesFile = async() => {
        try {
            ...
            const responseData = response.data
            setSizeFiles(responseData.size)
        }
    }
    useEffect(()=>{
        fetchPost()
        ...
        <>  
            ...
            <div className="bg-white rounded-md w-[600px] overflow-y-auto">
                <div className="flex justify-between text-xl mb-2 border-b-2 p-4">
                    <div className="flex items-center"><FaRegCommentAlt /><div className="ml-1">Edit Post {sizeFiles}</div></div>
