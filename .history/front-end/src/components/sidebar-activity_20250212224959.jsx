    ...
    return (
        ...
        <div className=" md:ml-32 ml-[6rem] bg-white border-r-2 md:w-72 h-screen fixed hidden lg:block bg-gray-50 overflow-y-auto " >
            ...
            {isLogin.role === 'student' && activities.map((data,id)=> (
                <div className={`p-3 flex justify-between text-sm border-b-2 border-gray-300 cursor-pointer hover:bg-gray-100 ${data.activity == 'No' ? 'bg-gray-200': ''}  ${selectedId == data.id ? 'bg-gray-100' : ''}`} key={id} onClick={()=>handleWorkId(data.id,data.assignment_type)}>
            ...
            {isLogin.role === 'teacher' && activities.map((data,id)=> (
                <div className={`p-3 flex justify-between text-sm border-b-2  cursor-pointer hover:bg-gray-100 ${selectedId == data.id ? 'bg-gray-100' : ''}`} key={id} onClick={()=>LinkPageAssignment(data.id_assignment,data.id_classroom)}>
                        <div className="mx-1"><div className="bg-sky-600 rounded p-2"><FaBook className="h-9 w-8 text-white" /></div></div>
                        <div className="mx-1 ">
                            <span className="line-clamp-2">{data.fname+' '+data.lname} Added a task</span>
...

        {
            sidebar &&(
                <div className={`md:ml-32 ml-[6rem] bg-white border-r-2 w-72 h-screen fixed bg-gray-50 lg:hidden block overflow-y-auto z-40 sidebar `}>
                    ...
                    {isLogin.role === 'student' && activities.map((data,id)=> (
                        <div className={`p-3 flex justify-between text-sm border-b-2  cursor-pointer hover:bg-gray-100 ${selectedId == data.id ? 'bg-gray-100' : ''}`} key={id} onClick={()=>handleWorkId(data.id,data.assignment_type)}>
                                <div className="mx-1"><div className="bg-sky-600 rounded p-2"><FaBook className="h-9 w-8 text-white" /></div></div>
                                <div className="mx-1 ">
                                    <span className="line-clamp-2">{data.fname+' '+data.lname} added an assignment</span>
                                    <span className="line-clamp-1 text-gray-500">{formattedDate(data.due_time)}</span>
