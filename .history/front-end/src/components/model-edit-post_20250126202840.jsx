...
const ModelEditPost = ({open,onClose,id,fetchAllPost}) => {
    const handleFetchData = () => {
        fetchPost(); // เรียก fetchPost จาก props
    };
    const modules = {
        toolbar: [
    ...
    const fetchSizesFile = async() => {
        try {
            const response = await axios.get(`/get-sizes-file/${id}`)
            const responseData = response.data
    ...
    console.log('Modules loaded',dataPost.message)
    const updatePost = async() => {
        ...
        if(isValid){
            ...
            const response = await axios.post('/edit-post',data,{
    ...
    const handleFileChange = (e) => {
        ...
        const newFiles = files.map((file) => {
            ...
            let extension = '';
            const existingFileNames = dataPost.file ? JSON.parse(dataAssignment.file) :[]
        ...
        <>  
            ...
            <div className="bg-white rounded-md w-[600px] overflow-y-auto">
                <div className="flex justify-between text-xl mb-2 border-b-2 p-4">
                    <div className="flex items-center"><FaRegCommentAlt /><div className="ml-1">Edit Post {totalSizeFiles}</div></div>
