...
const Work_sheet = ({isLogin,workId,sidebar,setSidebar}) =>{
    ...
    const fetchSizesFile = async() => {
        try {
            let dataId = work.filter(f => f.id == workId)
            const response = await axios.get(`/size-files-work/${dataId[0].id_assignment}/${workId}/:id`)
            const responseData = response.data
    ...
    console.log("work.filter(f => f.id == workId)", work.filter(f => f.id == workId)[0].id)
    useEffect(() => {
        // fetchSizesFile();
        setFileNames([]);
    }, [workId]);
