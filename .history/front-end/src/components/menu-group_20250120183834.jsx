...
const MenuGroup = ({menuGroup}) => {
    ...
    const handleRememberGroup = () => {
        const filtered = dataRememberGroup.filter(group =>
            student.some(st => st.id_user === group.id_user)
        );
        setMatchedUsers(filtered);
        // setCurrentPage(3)
    }
    // useEffect(()=> {
        console.log("matchedUsers"+dataRememberGroup.filter(group =>student.some(st => st.id_user === group.id_user)))
    // },[matchedUsers])
