...
const DisplayGroup = ({goToPreviousPage,GroupStudents=[],students,OnClose,setStudent = () => {},resetName=[]}) => {
    const [search, setSearch] = useState('')
    const [open,setOpen] = useState(false)
    const resetGroup = () => {
        // const name = GroupStudents.map((data)=>data.students.map((student)=>student.name))
        // const data = GroupStudents.flatMap(group => 
        //     group.students.map(student => ({
        //         name: student.name,
        //         id: student.id
        //     })))
        setStudent(resetName)
        goToPreviousPage()
        OnClose()
    }
    console.log("ddd"+GroupStudents.length)
    ...
    return(
        <>
                    ...
                    <>
                        <div className={`flex ${index === GroupStudents.length - 1 ? 'border-y-2' : 'border-t-2'} py-2 items-center`}>
                            ...
                            <div className="grow" key={index}>
                                <div >
                                    {group.fname+" "+group.lname}
                                </div>
                                <div className="text-sm text-gray-500">{group.students.length} member</div>
                                {group.students.map((student, index) =>(
                                    <div>{student.fname}{student.id_user}</div>
                                ))}
                            </div>
                            <div className="mr-3">
                                <GoPencil className=" w-5 h-5 cursor-pointer hover:text-sky-600 transition ease-in-out delay-150" />
                            </div>
                        </div>
                    </>
                ))
            ...
            <div className="flex justify-between mb-3 mt-4">
                ...
                <div><button className=" px-7 py-2 cursor-pointer hover:bg-sky-600 text-white bg-sky-500 transition ease-in-out delay-150">Done</button></div>
            </div>
