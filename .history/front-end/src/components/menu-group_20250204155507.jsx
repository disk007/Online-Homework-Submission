...
const MenuGroup = ({menuGroup}) => {
    ...
    const handleRememberGroup = () => {
        ...
        const groupedByGroupId = dataRememberGroup.reduce((acc, group) => {
            ...
            if (member) {
                acc[group.no_group].members.push(member);
            }
    
            return acc;
        }, {});
    
        const newGroups = Object.values(groupedByGroupId).filter(group => group.members.length > 0);
        // ถ้ามีกลุ่มที่มีสมาชิก, ตั้งค่า groups ใหม่
