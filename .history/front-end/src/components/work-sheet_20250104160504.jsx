...
const Work_sheet = ({isLogin,workId,sidebar,setSidebar}) =>{
    ...
    const handleFileChange = (e) => {
        ...
        const existingFileNames = myWork
  .filter(f => f.id == workId)
  .map(data => {
    try {
      return data.work ? JSON.parse(data.work) : null; // ตรวจสอบและแปลง JSON
    } catch (error) {
      console.error("Invalid JSON in data.work:", error);
      return null; // ถ้า JSON ไม่ถูกต้อง ให้คืนค่า null
    }
  })
  .filter(Boolean); // ลบค่า null หรือ undefined ออกจากผลลัพธ์
