...
const CreateGroup = ({goToNextPage,students=[],setStudent = () => {}, setGroups = () => {}}) =>{
    ...
    const createGroup = (e) => {
        ...
        if(isValid){
            setErrors({})
            goToNextPage()
            setGroups({name,students: selectedStudents.map(id => students.find(student => student.id === id))})
            setStudent(students.filter(student => !selectedStudents.includes(student.id)))
        }
        else{
            setErrors(validation)
        }
    }
    console.log("ddd "+students.length) 
    return(
        <>
            <div className="mt-5">
                <div>Group name</div>
            ...
            <form onSubmit={createGroup}>
                ...
                <div className="h-32 overflow-y-auto">
                    {students.filter((data) => data.fname && data.fname.toLowerCase().includes(search.toLowerCase())).map((student) => (
                        <div key={student.id_user} className="flex items-center py-2 hover:bg-gray-200 pl-2 cursor-pointer" onClick={() => handleSelectStudent(student.id_user, student.fname)}>
                            <input
                                type="checkbox"
                                name={student.fname}
                                className="w-4 h-4"
                                checked={selectedStudents.includes(student.id)}
                                onClick={() => handleSelectStudent(student.id_user, student.fname)}
                            />
