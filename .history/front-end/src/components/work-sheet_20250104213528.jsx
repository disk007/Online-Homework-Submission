...
const Work_sheet = ({isLogin,workId,sidebar,setSidebar}) =>{
    ...
    const handleFileChange = (e) => {
        ...
        const check = myWork.filter(f => f.id == workId).filter((data => data.work !== null))
        let existingFileNames = null
        // const existingFileNames = myWork.filter(f => f.id == workId).map(data => (JSON.parse(data.work)))
        if(check){
            existingFileNames =  JSON.parse(myWork.filter(f => f.id == workId).map((data => data.work)))
        }
        else{
            existingFileNames = []
        }
    ...
    const check = myWork.filter(f => f.id == workId)
        // const existingFileNames = myWork.filter(f => f.id == workId).map(data => (JSON.parse(data.work)))
        if(check){
           console.log("t "+check)
        }
