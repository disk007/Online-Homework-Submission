...
const ResetPassword = () => {
    ...
    const verifyToken = async () => {
        if(token !== null){
            const response = await axios.post('/verifyTokenPass',{token},{
                headers: {'Content-Type': 'application/json'}
            })
            const responseData = response.data
            setValid(responseData.valid)
        }
    };
    useEffect(() => {
        verifyToken();
    ...
    const resetPass = async(e) => {
        try {
            ...
            if(isValid){
                ...
                formData.append('pass', passwords.new)
    ...
    if (Valid === false) {
        return <div className="mt-12 flex justify-center items-center">Invalid, expired, or already used token. Please request a new password reset link.</div>;
    }
